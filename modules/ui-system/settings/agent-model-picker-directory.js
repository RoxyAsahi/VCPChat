...
export function createAgentModelPickerDirectory({ electronAPI, input }) {
    const modelOptions = async signal => {
        ...
        if (normalizeAgentModels(models).length === 0 && electronAPI?.refreshModels) {
            // The refresh IPC may return the authoritative model payload
            // before the renderer cache notification arrives. Prefer that
            // payload for this open transaction, then fall back to cache for
            // older contextBridge implementations that return only status.
            const refreshed = await electronAPI.refreshModels();
            if (signal.aborted) return [];
            models = normalizeAgentModels(refreshed).length > 0
                ? refreshed
                : await electronAPI.getCachedModels?.();
        }
        if (signal.aborted) return [];
