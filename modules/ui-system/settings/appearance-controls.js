...
export function mountAppearanceLanguageRows(form, api, scope) {
    ...
    fields.forEach(([id, title, description]) => {
        ...
        const row = api.mountLanguageRow(host, {
            title, description, options, activeId: select.value,
            onSelect: value => { select.value = value; select.dispatchEvent(new select.ownerDocument.defaultView.Event('change', { bubbles: true })); },
        }, scope);
...
export function mountChatFontRows(form, api, scope) {
    ...
    fields.forEach(([hostId, id, title, description]) => {
        ...
        select.dataset.vcpTypedPrimitiveMounted = 'true';
        const options = Array.from(select.options).map(option => ({ id: option.value, label: option.textContent || option.value }));
        const row = api.mountLanguageRow(host, { title, description, options, activeId: select.value, onSelect: value => { select.value = value; select.dispatchEvent(new select.ownerDocument.defaultView.Event('change', { bubbles: true })); } }, scope);
        scope.listen(select, 'change', () => row.setActive(select.value), undefined, `typed-${id}-language-row-sync`);
        scope.own(() => { delete select.dataset.vcpTypedPrimitiveMounted; }, `typed-${id}-language-row-marker`, 'ui-primitive');
    });
}
...
export function mountAppearanceRadiusLanguageRow(form, api, scope) {
    ...
    // Prevent the generic select projection from wrapping this canonical
    // control a second time. LanguageRow is the sole presentation owner.
    select.dataset.vcpTypedPrimitiveMounted = 'true';
    const options = Array.from(select.options).map(option => ({ id: option.value, label: option.textContent || option.value }));
    const row = api.mountLanguageRow(host, {
        title: '列表项圆角',
    ...
    scope.own(() => {
        delete host.dataset.vcpTypedPrimitiveMounted;
        delete select.dataset.vcpTypedPrimitiveMounted;
    }, 'typed-radius-language-row-marker', 'ui-primitive');
