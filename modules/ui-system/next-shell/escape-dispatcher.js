    ...
    class EscapeDispatcher {
        ...
        dispatch(event) {
            ...
            for (const entry of candidates) {
                let active = false;
                try { active = Boolean(entry.isActive()); } catch (error) {
                    this.warn('[NextUI] Escape owner activity check failed:', error);
                }
                if (!active) continue;
                try {
                ...
                // Escape is a single-owner action.  Other document-level
                // capture listeners (including legacy/WA modal handlers) must
                // not observe the same key and close a second surface.
