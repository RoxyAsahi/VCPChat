...
function createAgentActivityReadonlyView({ document = globalThis.document, actions = {} }) {
    ...
    function buildUsage(current) {
        ...
        identityStat('消息', `${messages.length}（用户 ${messages.filter((item) => item.role === 'user').length} / 助手 ${messages.filter((item) => item.role === 'assistant').length}）`);
        ...
        for (const [label, value] of [
            ...
            ['缓存读取', usage.cacheReadTokens], ['缓存写入', usage.cacheWriteTokens],
        ]) {
            const item = node('li', '', undefined, document);
            item.append(
                node('span', 'agent-chat-usage-label', label, document),
                node('span', 'agent-chat-usage-value', hasUsage && value != null ? format(value) : placeholder, document),
            );
            stats.append(item);
        }
        wrap.append(stats);
        const identityLabel = [usage.model, usage.provider].filter(Boolean).join(' · ');
        wrap.append(node('p', 'agent-chat-usage-note', `${usageSourceLabel}${identityLabel ? `；${identityLabel}` : ''}。此处是最近一次可靠报告，不伪装为 Session 累计费用。`, document));
        ...
        if (usage.inputTokens && messages.length) {
            ...
            const legend = node('div', 'agent-chat-context-breakdown-legend', undefined, document);
            let offset = 0;
            for (const [key, label] of [['system', '系统'], ['user', '用户'], ['assistant', '助手'], ['tool', '工具'], ['other', '其他']]) {
                ...
                segment.setAttribute('width', String(percent));
                segment.setAttribute('height', '8');
                segment.setAttribute('aria-label', `${label} ${percent}%`);
                bar.append(segment);
                offset += percent;
                const item = node('span', `agent-chat-context-legend-item is-${key}`, undefined, document);
                item.append(node('i', '', undefined, document), document.createTextNode(`${label} ${percent}%`));
                legend.append(item);
            }
        ...
        if (instructionMode === 'codex-managed') {
            ...
            body.append(
                node('p', 'agent-chat-usage-note', '基础身份由 Codex App Server 0.146 管理；协议不返回完整内部 prompt，VChat 不伪造展示。', document),
                node('p', 'agent-chat-usage-note', `Personality：${snapshot.personality || 'none'}`, document),
            );
            ...
            const details = node('details', 'agent-chat-context-prompt', undefined, document);
            details.append(
                node('summary', 'agent-chat-context-section-title', '有效指令（VChat 身份）', document),
                node('pre', 'agent-chat-toolbox-ws-output', String(prompt).slice(0, 32_768), document),
            );
            wrap.append(details);
        }
