(() => {
    ...
    function closeAllInternalApps() {
        [...views.keys()].forEach(closeView);
        closeLaunchpad();
        window.VCPUI?.feedback.cancelAll();
        getDesktopApi()?.desktopCloseAllEmbeddedVchatApps?.().catch(error => {
    ...
    function setupAccountMenu() {
        ...
        window.addEventListener('global-settings-updated', sync);
        window.addEventListener('ui-mode-changed', event => {
            if (event.detail?.mode !== 'next') setOpen(false);
        });
        sync();
    }
    ...
    async function openCreateDialog() {
        ...
        const nameInput = nameControl.element.querySelector('input');
        ...
        const submit = async () => {
            ...
            const model = modelControl.element.value;
    ...
    function setupAgentSearch() {
        ...
        trigger.addEventListener('click', () => setSearchMode(true, false));
        close.addEventListener('click', () => setSearchMode(false));
        ...
        window.addEventListener('ui-mode-changed', event => {
            if (event.detail?.mode !== 'next') setSearchMode(false);
        });
    }
    ...
    function init() {
        ...
        document.getElementById('nextUiThemeStoreBtn')?.addEventListener('click', () => (window.chatAPI || window.electronAPI)?.openThemesWindow?.());
        document.getElementById('nextUiThemeBtn')?.addEventListener('click', () => proxyClick('themeToggleBtn'));
        document.getElementById('nextUiSettingsBtn')?.addEventListener('click', () => window.uiHelperFunctions?.openModal?.('globalSettingsModal'));
