...
export function mountDirectoryBrowser(props: DirectoryBrowserProps, scope: UiScope): DirectoryBrowserController {
    ...
    let generation = 0; let controller: AbortController | null = null; let parent: DirectoryBrowserListing | null = null; let selected: DirectoryBrowserEntry | null = null; let child: DirectoryBrowserListing | null = null; let loading = false; let busy = Boolean(props.busy); let showHidden = false; let failure: string | null = null; let creating = false; let editingPath = false; let pathDraft = '';
    const modal = mountModal({ title: props.title ?? 'Open folder', className: 'vcp-directory-browser', body: frame, headless: true, open: props.open, onClose: () => { if (!busy && !creating) props.onClose(); } }, browserScope);
    ...
    const sync = () => {
        ...
        if (editingPath) {
            const input = document.createElement('input'); input.type = 'text'; input.className = 'vcp-directory-browser-path-input'; input.value = pathDraft; input.setAttribute('aria-label', 'Folder path'); input.disabled = busy || loading || creating;
            ...
            const edit = document.createElement('button'); edit.type = 'button'; edit.className = 'vcp-directory-browser-path-edit'; edit.textContent = '✎'; edit.setAttribute('aria-label', 'Edit folder path'); edit.disabled = !source || busy || loading || creating; browserScope.listen(edit, 'click', () => { if (!source) return; editingPath = true; pathDraft = source.path.endsWith('/') || source.path.endsWith('\\') ? source.path : `${source.path}/`; sync(); }); crumbs.append(edit);
        }
        ...
        hidden.setAttribute('aria-pressed', String(showHidden)); hidden.disabled = busy || creating;
        create.disabled = !parent || loading || busy || creating;
        cancel.disabled = busy || creating; confirm.disabled = !parent || loading || busy || creating;
    };
    ...
    browserScope.listen(create, 'click', () => { if (!parent || creating) return; const name = 'New folder'; creating = true; sync(); const target = selected?.path ?? parent.path; const token = generation; void props.createDirectory(target, name).then(created => { if (token !== generation || !modal.open) return; creating = false; navigate(target); selected = { name, path: created }; }, reason => { if (token !== generation || !modal.open) return; creating = false; failure = errorText(reason); sync(); }); });
    const setOpen = (open: boolean) => { if (open) { modal.setOpen(true); parent = null; selected = null; child = null; failure = null; showHidden = false; editingPath = false; navigate(); } else { generation += 1; controller?.abort(); controller = null; editingPath = false; modal.setOpen(false); } };
