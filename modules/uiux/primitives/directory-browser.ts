...
export function mountDirectoryBrowser(props: DirectoryBrowserProps, scope: UiScope): DirectoryBrowserController {
    ...
    const navigate = (path?: string) => { selected = null; child = null; void scan(path, listing => { parent = listing; }); };
    ...
    const setOpen = (open: boolean) => { if (open) { modal.setOpen(true); parent = null; selected = null; child = null; failure = null; showHidden = false; navigate(); } else { generation += 1; controller?.abort(); controller = null; modal.setOpen(false); } };
