...
function ensureStyles() {
    ...
    style.id = STYLE_ID;
    style.textContent = `.vcp-harness-select{position:relative;display:inline-flex;min-width:12rem}.vcp-harness-select>.vcp-harness-select-native{position:absolute!important;width:1px!important;height:1px!important;opacity:0!important;pointer-events:none!important}.vcp-harness-select>.vcp-harness-select-trigger{display:inline-flex;align-items:center;justify-content:space-between;gap:8px;width:100%;min-height:40px;padding:8px 10px;border:1px solid var(--vcp-color-border,#c8ccd4);border-radius:10px;background:var(--vcp-color-surface,#fff);color:var(--vcp-color-text,#1f2329);font:inherit;font-size:14px;line-height:22px;cursor:pointer}.vcp-harness-select>.vcp-harness-select-trigger:focus-visible{outline:2px solid var(--vcp-color-focus,#4c8dff);outline-offset:2px}.vcp-harness-select>.vcp-harness-menu-list,.vcp-harness-menu-list{z-index:1000;min-width:100%;padding:4px;background:var(--vcp-color-surface,#fff);border:1px solid var(--vcp-color-border,#c8ccd4);border-radius:10px;box-shadow:0 8px 24px rgb(0 0 0/.14)}.vcp-harness-menu-list .vcp-harness-menu-item{display:flex;align-items:center;width:100%;min-height:40px;padding:8px 10px;border:0;border-radius:8px;background:transparent;color:inherit;font:inherit;font-size:14px;line-height:22px;text-align:left;cursor:pointer}.vcp-harness-menu-list .vcp-harness-menu-item:hover:not(:disabled),.vcp-harness-menu-list .vcp-harness-menu-item:focus-visible,.vcp-harness-menu-list .vcp-harness-menu-item[data-selected=true]{background:var(--vcp-color-surface-selected,#eef3ff)}.vcp-harness-menu-list .vcp-harness-menu-item:disabled{opacity:.5;cursor:not-allowed}.vcp-harness-field-description{margin-top:4px;color:var(--vcp-color-muted,#68707d);font-size:12px;line-height:18px}.vcp-harness-field-error{margin-top:4px;color:var(--vcp-color-danger,#c62828);font-size:12px;line-height:18px}`;
...
export function mountSelect(select: HTMLSelectElement, props: SelectProps = {}, scope: UiScope): UiDisposer {
    ...
    const menu = document.createElement('div'); menu.className = 'vcp-harness-menu-list'; menu.setAttribute('role', 'menu'); menu.hidden = true;
    ...
    const sync = () => {
        ...
        Array.from(menu.querySelectorAll<HTMLElement>('[role="menuitem"]')).forEach((item, index) => {
            const active = index === select.selectedIndex; item.dataset.selected = String(active); item.setAttribute('aria-checked', String(active)); item.tabIndex = active ? 0 : -1;
        });
    };
    ...
    Array.from(select.options).forEach((option, index) => {
        ...
        scope.listen(item, 'click', () => { if (!option.disabled) { select.selectedIndex = index; select.dispatchEvent(new Event('change', { bubbles: true })); close(); trigger.focus(); } }); menu.append(item);
    });
