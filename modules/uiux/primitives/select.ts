...
export function mountSelect(select: HTMLSelectElement, props: SelectProps = {}, scope: UiScope): UiDisposer {
    ...
    Array.from(select.options).forEach((option, index) => {
        ...
        const item = document.createElement('button'); item.type = 'button'; item.className = 'vcp-harness-menu-item'; item.setAttribute('role', 'menuitem'); item.disabled = option.disabled; const label = document.createElement('span'); label.className = 'vcp-harness-menu-item-label'; label.textContent = option.textContent?.trim() || ''; item.append(label); itemWrap.append(item);
