...
export function mountSelect(select: HTMLSelectElement, props: SelectProps = {}, scope: UiScope): UiDisposer {
    ...
    const sync = () => {
        ...
        Array.from(menu.querySelectorAll<HTMLElement>('[role="menuitem"]')).forEach((item, index) => {
            const active = index === select.selectedIndex; item.dataset.selected = String(active); item.classList.toggle('vcp-harness-menu-item-selected', active); item.setAttribute('aria-checked', String(active)); item.tabIndex = active ? 0 : -1; const check = item.querySelector<HTMLElement>('.vcp-harness-menu-item-check'); if (check) check.toggleAttribute('hidden', !active);
        });
    };
    ...
    Array.from(select.options).forEach((option, index) => {
        ...
        const item = document.createElement('button'); item.type = 'button'; item.className = 'vcp-harness-menu-item'; item.setAttribute('role', 'menuitem'); item.disabled = option.disabled; const label = document.createElement('span'); label.className = 'vcp-harness-menu-item-label'; label.textContent = option.textContent?.trim() || ''; item.append(label); const check = document.createElementNS('http://www.w3.org/2000/svg', 'svg'); check.classList.add('vcp-harness-menu-item-check'); check.setAttribute('aria-hidden', 'true'); check.setAttribute('viewBox', '0 0 16 16'); check.setAttribute('focusable', 'false'); check.innerHTML = '<path d="M3 8.5 6.5 12 13 4.5" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>'; check.toggleAttribute('hidden', true); item.append(check); itemWrap.append(item);
