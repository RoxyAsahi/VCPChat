...
export function mountAgentModelPicker(host: HTMLElement, props: AgentModelPickerProps, scope: UiScope): AgentModelPickerController {
    ...
    pickerScope.listen(paneCell, 'click', () => {
        pane = 'model';
        // Root -> model is a new PopupSelect command segment. Re-open the
        // controller so its async directory snapshot is loaded for this pane
        // instead of merely revealing an empty viewport from the root pass.
        popup.open('agent-model', {}, { via: 'menu', span: { source: 'agent-model-picker-model-pane' } });
        syncPane();
    });
