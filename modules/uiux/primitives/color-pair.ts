...
export function mountColorPair(color: HTMLInputElement, text: HTMLInputElement, scope: UiScope): UiDisposer {
    ...
    const onText = () => { if (valid(text.value)) color.value = text.value; else text.value = color.value; };
    scope.listen(color, 'input', onColor); scope.listen(color, 'change', onColor); scope.listen(text, 'change', onText); syncText();
