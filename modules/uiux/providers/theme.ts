...
function applySemanticTokens(root: HTMLElement, effective: ThemeState['effective']): () => void {
    ...
    return () => {
        previous.forEach((value, name) => {
            if (value) tokenRoot.style.setProperty(name, value);
            else tokenRoot.style.removeProperty(name);
        });
        if (previousScheme) tokenRoot.style.setProperty('color-scheme', previousScheme);
        else tokenRoot.style.removeProperty('color-scheme');
    };
}
