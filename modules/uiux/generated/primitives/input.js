...
export function mountInput(input, props = {}, scope) {
    ...
    return scope.own(() => { if (input.parentNode === wrap)
        parent.insertBefore(input, wrap); wrap.remove(); if (!originalInputClass)
        input.classList.remove('input'); }, 'harness-input', 'ui-primitive');
}
