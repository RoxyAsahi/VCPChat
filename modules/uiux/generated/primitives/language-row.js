...
export function mountLanguageRow(host, props, scope) {
    ...
    const rebuild = async () => { await menuScope.dispose('harness-language-row-menu-rebuild'); menuScope = scope.child('harness-language-row-menu'); buildMenu(); sync(); };
    const dispose = scope.own(async () => { await rowScope.dispose('harness-language-row-unmounted'); await menuScope.dispose('harness-language-row-menu-unmounted'); row.remove(); host.replaceChildren(...originalChildren); }, 'harness-language-row', 'ui-primitive');
    return { root: row, trigger, get menu() { return menuController; }, get open() { return menuController.open; }, setOptions: async (next) => { options = [...next]; await rebuild(); }, setActive(next) { activeId = next ?? ''; sync(); }, setLoading(next) { loading = Boolean(next); sync(); }, setOpen(value) { menuController.setOpen(value); }, dispose };
}
