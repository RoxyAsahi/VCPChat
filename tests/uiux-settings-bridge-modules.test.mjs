...
test('Agent ModelPicker directory consumes refresh payload before cache notification', async () => {
    const { createAgentModelPickerDirectory } = await import(
        pathToFileURL(path.join(settingsDir, 'agent-model-picker-directory.js')).href,
    );
    const api = {
        async getCachedModels() { return []; },
        async refreshModels() { return { models: [{ id: 'fresh-model', name: 'Fresh Model' }] }; },
        async getHotModels() { return ['fresh-model']; },
        async getFavoriteModels() { return []; },
    };
    const directory = createAgentModelPickerDirectory({ electronAPI: api, input: { value: '' } });
    const options = await directory.options(new AbortController().signal);
    assert.deepEqual(options.map(option => [option.group, option.id]), [
        ['热门模型', 'fresh-model'], ['全部模型', 'fresh-model'],
    ]);
});

test('topic summary model reuses the typed picker without replacing its native value node', () => {
    const entry = read(bridgeEntry);
    assert.match(entry, /function mountTypedTopicSummaryModelPicker\(form\)/);
    assert.match(entry, /inputId: 'topicSummaryModel'/);
    assert.match(entry, /triggerId: 'openTopicSummaryModelSelectBtn'/);
    assert.match(entry, /scopeLabel: 'topic-summary-model-picker-production'/);
    assert.match(entry, /mountTypedTopicSummaryModelPicker\(form\)/,
        'global Settings must mount the topic-summary picker on the real form');
    assert.match(entry, /input\.value = option\.id/,
        'picker selection must write the canonical native input rather than a duplicate store');
});

test('legacy model picker handlers are fallback-only when typed triggers are mounted', () => {
    const manager = read(path.join(root, 'modules/settingsManager.js'));
    assert.match(manager, /vcpTypedAgentModelPicker === 'true'/,
        'Agent legacy model handler must yield to the typed picker marker');
    assert.match(manager, /vcpTypedTopicSummaryModelPicker === 'true'/,
        'topic summary legacy model handler must yield to the typed picker marker');
    assert.match(manager, /compatibility fallback|fallback for Classic\/bootstrap paths/i,
        'legacy model handler must document its fallback-only boundary');
});
...
test('hidden Agent actions cannot be resurrected by generated Button mounting', () => {
    const entry = read(bridgeEntry);
    const buttonOwner = entry.match(/function mountTypedAgentButtons\(form\)\s*\{([\s\S]*?)\n\}/)?.[1] || '';
    assert.match(buttonOwner, /button\.hidden\s*\|\|/, 'hidden compatibility actions must be skipped by the Button adapter');
    assert.match(buttonOwner, /Submit\/delete remain canonical compatibility commands/, 'the skip must document autosave ownership');
});

test('compact Agent disclosure summaries use an independent second grid row', () => {
    const overrides = read(path.join(root, 'styles', 'ui-system', 'settings-overrides.css'));
    assert.match(overrides, /#tabContentSettings #agentSettingsContainer \.agent-settings-section-header[\s\S]*?grid-template-areas: "title toggle" "summary summary" !important/,
        'narrow Settings summaries must not share the title row');
    assert.match(overrides, /#tabContentSettings #agentSettingsContainer \.agent-settings-section-summary[\s\S]*?grid-column: 1 \/ -1 !important/,
        'summary must span the full compact row');
});
...
test('global typed primitive mounts keep one lifecycle registration per primitive', () => {
    ...
    for (const primitive of ['mountChoice', 'mountNumericStepperRow', 'mountToggle', 'mountColorPair', 'mountInput', 'mountSelect']) {
