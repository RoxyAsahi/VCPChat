...
test('voice input adapter resolves the deployed native sidecar', t => {
    const adapter = new VoiceInputEngineAdapter({ projectRoot });
    const executablePath = adapter.resolveExecutablePath();

    // Release artifacts are platform-specific and are generated during the
    // packaging build. Keep source-only CI on an unsupported host explicit
    // without pretending that another platform's binary is equivalent.
    if (!executablePath) {
        t.skip(`voice input runtime is not built for ${process.platform}-${process.arch}`);
        return;
    }
...
test('voice input sidecar starts on demand, answers ping, and shuts down', async t => {
    const adapter = new VoiceInputEngineAdapter({ projectRoot });
    if (!adapter.resolveExecutablePath()) {
        t.skip(`voice input runtime is not built for ${process.platform}-${process.arch}`);
        return;
    }
    t.after(async () => {
