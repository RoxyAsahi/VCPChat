...
test('Model picker B1 state matrix stays a reproducible Candidate-Lab gate', () => {
    ...
    assert.equal(report.harnessProductionFailureToast.captured, true);
    assert.equal(report.harnessProductionFailureToast.productionComparison, false);
    assert.ok(report.missingEvidence.includes('same-semantic VCP selection-error Toast visual comparison'));
...
test('Paired evidence ledger keeps Candidate captures and blocked boundaries explicit', () => {
    ...
    assert.equal(report.candidateCaptures.filter(item => item.name !== 'state-dot').every(item => item.state === 'vcp-candidate-capture-only' && item.captured && item.sourceContractPresent && item.contractPass), true);
    assert.equal(report.candidateCaptures.find(item => item.name === 'state-dot')?.state, 'paired-real-source-boundary-recorded');
    assert.equal(report.candidateCaptures.find(item => item.name === 'state-dot')?.contractPass, true);
...
test('Real-source diff ledger records cross-component boundaries without promotion', () => {
    ...
    assert.equal(report.pass, false);
    assert.equal(report.counts.realSourceDiffs, 11);
    assert.equal(report.counts.semanticFixtureMatches, 11);
    assert.equal(report.counts.parityPasses, 0);
    assert.equal(report.counts.pixelMeasured, 11);
...
test('Menu Candidate capture records portal, interaction states, and teardown', () => {
    ...
    assert.equal(report.escapeClosed, true);
    assert.deepEqual(report.disposed, { restored: true });
});
