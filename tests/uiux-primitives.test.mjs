...
test('Harness FontSizeRow owns a stepped presentation over the canonical scale select', async () => {
    const dom = new JSDOM('<!doctype html><main><div id="host"><select id="font"><option value="small">较小</option><option value="normal" selected>标准</option><option value="large">较大</option></select></div></main>');
    const previousDocument = globalThis.document; const previousWindow = globalThis.window;
    globalThis.document = dom.window.document; globalThis.window = dom.window;
    try {
        const scope = createUiScope(new LifecycleScope('font-size-row-test'));
        const host = document.getElementById('host'); const select = document.getElementById('font');
        const controller = mountFontSizeRow(host, select, scope);
        assert.equal(controller.root.className, 'vcp-harness-font-size-row');
        assert.equal(controller.root.querySelector('.vcp-harness-font-size-row-value')?.textContent, '14');
        assert.equal(controller.root.querySelectorAll('.vcp-harness-font-size-row-arrow svg').length, 2, 'FontSizeRow uses the Harness up/down chevron icons');
        controller.root.querySelector('.vcp-harness-font-size-row-arrow')?.dispatchEvent(new dom.window.MouseEvent('click', { bubbles: true }));
        assert.equal(select.value, 'large');
        assert.equal(controller.root.querySelector('.vcp-harness-font-size-row-value')?.textContent, '16');
        await scope.dispose('font-size-row-complete');
        assert.equal(host.querySelector('#font'), select);
        assert.equal(host.querySelector('.vcp-harness-font-size-row'), null);
    } finally { globalThis.document = previousDocument; globalThis.window = previousWindow; dom.window.close(); }
});
...
test('Harness AgentModelPicker maps provider metadata and retracts its popup owner', async () => {
    ...
    try {
        ...
        assert.equal(loads, 4,
            'root-to-model pane and post-Escape reopen each start a fresh catalog load');
        controller.close();
