...
function buildNextNotes() {
    ...
    const replaceButton = (id, fallbackLabel, icon, variant = 'secondary', size = 'sm') => {
        const original = document.getElementById(id);
        if (!original || !original.isConnected) return null;
    ...
    replaceIconButton('editorFindNext', 'chevron_down', '下一个匹配');
    replaceIconButton('editorFindClose', 'close', '关闭查找');

    // 编辑区工具栏按钮。
    replaceButton('newMdBtn', '新建MD', 'note_add');
    replaceButton('newTxtBtn', '新建TXT', 'description');
    replaceButton('newFolderBtn', '新建文档', 'create_new_folder');
    replaceButton('saveNoteBtn', '保存', 'save', 'primary');
    replaceButton('deleteNoteBtn', '删除', 'delete', 'danger');
