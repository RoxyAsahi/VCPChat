...
if (matrix.stateSemantics?.['select/focus'] !== 'AgentPresetSeat keyboard-open focus owner is captured independently; current VCP Select behavior is a known mismatch until an explicit interaction-contract decision') fail('Select focus state must preserve the captured interaction mismatch');
...
if (matrix.status?.selectMenuKeyboardFocus !== 'mismatch (Harness retains trigger focus; VCP transfers focus to selected menu item)') fail('Select focus checkpoint must retain the unresolved owner mismatch');
