...
if (interaction['directory-browser/injected-listing-selection-hidden-path-submit-abort-close-reopen-dispose']?.status !== 'foundation-electron-active') fail('DirectoryBrowser must retain its Electron-backed foundation status');
...
if (!String(matrix.status?.directoryBrowserFoundation).includes('Harness draft preview/filter, nested create dialog, slow-scan, VCP production consumer and same-semantic Harness pixel diff pending')) fail('DirectoryBrowser must retain its explicit incomplete-state ledger');
