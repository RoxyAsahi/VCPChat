...
const normalizeClass = value => [...new Set(String(value || '').split(/\s+/).filter(Boolean).map(token => {
    ...
    return token.replace(/^vcp-uiux-input-wrap$/, 'wrap').replace(/^vcp-harness-menu-list$/, 'list').replace(/^vcp-harness-menu-viewport$/, 'viewport').replace(/^vcp-harness-menu-item-wrap$/, 'itemWrap').replace(/^vcp-harness-menu-item$/, 'item');
}))].sort().join(' ');
...
for (const [name, file] of cases) {
  const harness = shape(parse(path.join(fixtureRoot, 'harness', file)));
  const vcp = shape(parse(path.join(fixtureRoot, 'vcp', file)));
