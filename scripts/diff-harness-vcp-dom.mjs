...
const normalizeClass = value => [...new Set(String(value || '').split(/\s+/).filter(Boolean).map(token => {
    ...
    return token.replace(/^vcp-uiux-input-wrap$/, 'wrap').replace(/^vcp-harness-menu-list$/, 'list').replace(/^vcp-uiux-primitive-menu$/, '').replace(/^vcp-harness-menu-viewport$/, 'viewport').replace(/^vcp-harness-menu-item-wrap$/, 'itemWrap').replace(/^vcp-harness-menu-item$/, 'item').replace(/^vcp-harness-menu-item-label$/, 'itemLabel').replace(/^vcp-harness-menu-item-check$/, 'check').replace(/^vcp-harness-menu-item-selected$/, 'selected');
}))].sort().join(' ');
