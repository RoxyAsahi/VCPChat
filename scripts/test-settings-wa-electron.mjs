...
try {
    ...
    const typedPrimitiveEvidence = await page.evaluate(() => {
        ...
        const root = select?.closest('.vcp-harness-language-row');
        const trigger = root?.querySelector('.vcp-harness-language-row-selector');
        trigger?.click();
        ...
        return {
            field: Boolean(root?.classList.contains('vcp-harness-language-row')),
    ...
    assert.equal(typedPrimitiveEvidence.field, true, 'typed LanguageRow owns appearance density root');
    ...
    assert.equal(typedPrimitiveEvidence.item, true, 'typed LanguageRow exposes menuitem contract');
    await page.evaluate(() => document.querySelector('#appearanceDensity')?.closest('.vcp-harness-language-row')?.querySelector('.vcp-harness-language-row-selector')?.click());
    console.log(`  [PASS] 1d. typed LanguageRow DOM and geometry contract ${JSON.stringify(typedPrimitiveEvidence)}`);
    ...
    const portalStacking = await page.evaluate(async () => {
        const densityTrigger = document.querySelector('#appearanceDensity')?.closest('.vcp-harness-language-row')?.querySelector('.vcp-harness-language-row-selector');
    ...
    assert.equal(await page.$eval('#appearanceFontScale', node => Boolean(node.closest('.vcp-harness-font-size-row')?.querySelector('.vcp-harness-font-size-row-stepper'))), true, 'typed appearanceFontScale FontSizeRow is mounted');
    assert.equal(await page.$eval('#appearanceFontScale', node => Boolean(node.closest('.vcp-harness-select-wrap'))), false, 'typed appearanceFontScale legacy Select wrapper is deleted');
