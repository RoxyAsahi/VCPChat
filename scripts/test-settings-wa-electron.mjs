...
try {
    ...
    assert.equal(shellState.navCount, 7, '7 categories in VCPUI List nav');
    ...
    assert.equal(shellState.sectionIds.length, 7, '7 setting sections present');
    ...
    await new Promise(resolve => setTimeout(resolve, 80));
    assert.equal(await page.evaluate(() => document.querySelectorAll('#globalSettingsModal .vcp-ui-list-item').length), 7, 'clearing search restores the full nav');
    ...
    const darkHash = (await fs.readFile(darkShot)).toString('base64').slice(0, 64);
    const lightHash = (await fs.readFile(lightShot)).toString('base64').slice(0, 64);
