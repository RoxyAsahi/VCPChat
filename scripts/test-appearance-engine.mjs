...
const raceDom = new JSDOM('<!doctype html><html><head></head><body></body></html>', {
...
assert.ok(
    ...
    'a deferred optics mount completes once the body exists'
);
raceDom.window.close();

const dom = new JSDOM('<!doctype html><html><body><div class="vcp-ui-scope"></div></body></html>', {
    url: 'https://vcpchat.local/',
    runScripts: 'outside-only'
});
...
assert.equal(appearance.normalize({ radius: 'invalid' }).radius, 'medium');
