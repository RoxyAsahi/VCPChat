...
try {
    ...
    const primitiveBoundary = await page.evaluate(() => {
        ...
        const result = { trigger: trigger?.getAttribute('aria-haspopup'), menu: document.querySelector('.vcp-harness-menu-list[role="menu"]') !== null, viewport: document.querySelector('.vcp-harness-menu-list .vcp-harness-menu-viewport') !== null, itemWrap: document.querySelector('.vcp-harness-menu-list .vcp-harness-menu-item-wrap') !== null, item: item?.getAttribute('role'), minHeight: style?.minHeight, padding: style?.padding, expanded: trigger?.getAttribute('aria-expanded'), inputWrap: input?.parentElement?.className, inputWrapHeight: inputWrapStyle?.height, inputWrapGap: inputWrapStyle?.gap, inputWrapRadius: inputWrapStyle?.borderRadius, inputWrapPadding: inputWrapStyle?.padding, inputFontSize: inputStyle?.fontSize, inputLineHeight: inputStyle?.lineHeight, choiceClass: choice.classList.contains('vcp-uiux-choice'), choiceValue: choice.dataset.value, rangeWrap: range?.parentElement?.className, rangeOutput: rangeOutput?.textContent, toggleWrap: toggle?.parentElement?.className, toggleChecked: toggle?.checked, legacySliderDisplay: legacySlider?.style.display, colorPairWrap: color?.parentElement?.className, colorValue: color?.value, colorText: colorText?.value };
