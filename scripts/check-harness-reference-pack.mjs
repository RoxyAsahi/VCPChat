...
const primitives = ['settings-root', 'field', 'select', 'menu', 'modal', 'tooltip', 'hover-card', 'disclosure-row', 'state-dot', 'toast', 'risk-confirmation', 'semantic-icon', 'button', 'input', 'range', 'toggle', 'color-pair', 'agent-preset-seat', 'agent-preset-row', 'agent-preset-section', 'agent-preset-label', 'preset-menu', 'popup-select', 'directory-browser', 'model-picker', 'diff-block', 'brand-wordmark', 'drop-overlay', 'attachment-rail', 'image-lightbox', 'fish-logo', 'popup-select-view', 'menu-view', 'app-frame', 'job-list-action', 'permission-row', 'produced-files', 'message-image', 'image-gallery', 'plan-chip', 'json-tree', 'read-block', 'search-block', 'terminal-block', 'web-block'];
const required = [
  'reference.css',
