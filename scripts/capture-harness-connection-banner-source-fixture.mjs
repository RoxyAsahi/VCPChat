...
try {
  ...
  const states = await page.evaluate(async () => { const capture = () => { const el = document.querySelector('#root>div'); if (!el) return { present: false }; const style = getComputedStyle(el), rect = el.getBoundingClientRect(); return { present: true, tag: el.tagName.toLowerCase(), text: el.textContent, aria: { role: el.getAttribute('role'), live: el.getAttribute('aria-live') }, rect: { width: rect.width, height: rect.height }, style: { position: style.position, top: style.top, left: style.left, right: style.right, zIndex: style.zIndex, padding: style.padding, textAlign: style.textAlign, fontSize: style.fontSize, lineHeight: style.lineHeight, backgroundColor: style.backgroundColor, color: style.color } }; }; const f = globalThis.__connectionBanner; const connectedHidden = capture(); f.render(true, '连接已断开，正在重连…'); await new Promise(requestAnimationFrame); const reconnectingVisible = capture(); f.render(true, 'Reconnecting to Harness…'); await new Promise(requestAnimationFrame); const labelUpdate = capture(); f.root.unmount(); return { connectedHidden, reconnectingVisible, labelUpdate, unmounted: { rootEmpty: document.querySelector('#root').childNodes.length === 0 } }; });
