...
const html = `<!doctype html><html><head><meta charset="utf-8"><style>html,body{margin:0;width:100%;height:100%;background:#fff}body{font-family:system-ui;color:#0f1115}.fixture{display:flex;gap:12px;align-items:center;padding:24px}</style><script type="module" src="/modules/uiux/generated/browser-entry.js"></script></head><body><div class="fixture"></div></body></html>`;
...
try {
    ...
    const evidence = await page.evaluate(() => {
        ...
        return { source: 'VCP generated artifact Candidate Lab', semanticFixture: 'portable-button/all-variants-and-disabled', viewport: { width: 800, height: 600, deviceScaleFactor: 1 }, cases: nodes, ownerRegistrations: releases.length };
    });
    assert.equal(evidence.cases.length, 6);
    await fs.mkdir(path.join(root, 'reports'), { recursive: true });
    await fs.writeFile(path.join(root, 'reports', 'vcp-button-candidate.json'), `${JSON.stringify(evidence, null, 2)}\n`);
    await page.screenshot({ path: path.join(root, 'reports', 'vcp-button-candidate.png') });
    ...
    assert.equal(disposed.count, 6);
    assert.equal(disposed.restored, true, 'Button fixture owner must restore each native button after capture');
    console.log(`VCP Button fixture captured (${evidence.cases.length} states; 800x600 @1x).`);
} finally {
    await browser.close();
