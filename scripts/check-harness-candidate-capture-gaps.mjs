...
const specs = [
  { name: 'agent-preset-seat', source: ['packages/client/ui-agent-preset/src/client/AgentPresetSeat.tsx', 'packages/client/ui-agent-preset/src/client/PresetMenu.tsx'], candidate: 'modules/uiux/primitives/agent-preset-seat.ts', boundary: 'VCP assistantAgent/chat switching chain is frozen', gaps: ['real Harness source browser capture', 'same-semantic DOM/ARIA and computed-style diff', 'same-semantic pixel diff'] },
  { name: 'agent-preset-row', source: ['packages/client/ui-agent-preset/src/client/AgentPresetRow.tsx', 'packages/client/ui-agent-preset/src/client/PresetMenu.tsx'], candidate: 'modules/uiux/primitives/agent-preset-row.ts', boundary: 'VCP has no equivalent non-frozen preset preference business capability', gaps: ['real Harness source browser capture', 'same-semantic DOM/ARIA and computed-style diff', 'same-semantic pixel diff'] },
  { name: 'popup-select', source: ['packages/client/ui-commands/src/client/popup.ts', 'packages/client/ui-commands/src/client/PopupSelectView.tsx'], candidate: 'modules/uiux/primitives/popup-select.ts', boundary: 'Composer/input overlay and command wiring are frozen', gaps: ['real Harness PopupSelectView browser capture', 'same-semantic DOM/ARIA and computed-style diff', 'same-semantic pixel diff'] },
  { name: 'disclosure-row', source: ['packages/client/ui-primitives/src/DisclosureRow.tsx'], candidate: 'modules/uiux/primitives/disclosure-row.ts', boundary: 'VCP chat/message consumer is frozen', gaps: ['same-semantic Harness/VCP pixel diff', 'VCP production consumer'] },
];
...
const entries = specs.map(spec => {
  ...
  return { ...spec, sourcePass, candidatePass, referencePass: Boolean(dom.provenance?.sources?.length && geometry.styleSource), candidateStatus: status, captureGap: /pending|frozen/.test(status), pass: false };
});
