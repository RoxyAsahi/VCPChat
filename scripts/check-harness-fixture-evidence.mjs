...
const report = { generatedAt: new Date().toISOString(), present: { ...present, vcpSelect: vcpSelectValid }, harnessSelectStatus: present.harnessSelect ? 'available' : 'pending', vcpSelectStatus: vcpSelectValid ? 'available' : 'pending-invalid-or-missing', geometryStatus: geometry.status, geometryPass: geometry.pass === true, pixelStatus: pixel.status, pixelPass: pixel.pass === true, pass: present.harnessInput && present.vcpGeometry && present.harnessSelect && vcpSelectValid && geometry.pass === true && pixel.pass === true };
