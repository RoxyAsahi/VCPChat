...
assert.equal(fixture.protocolRevision, '1.7');
...
assert.equal(ready.protocolRevision, '1.7');
assert.match(ready.buildRevision, /^[0-9a-f]{7,64}$/i);
const control = fixture.daemonToHost.find((message) => message.type === 'control-event' && message.kind === 'topics');
assert.equal(control.requestId, 'topics_1');
const createdTopic = fixture.daemonToHost.find((message) => message.type === 'control-event' && message.kind === 'topic-created');
assert.deepEqual(createdTopic.payload, {
    topicId: 'topic_new', agentId: 'Nova', title: '新的 Nova Topic',
    model: 'gpt-5.6-terra', workspaceRoot: 'C:\\VCP', readOnly: true,
});
const event = fixture.daemonToHost.find((message) => message.type === 'event').event;
for (const field of ['eventId', 'sequence', 'timestamp', 'runtime', 'sessionId', 'topicId', 'messageId']) assert.ok(event[field] !== undefined, `v1.7 event requires ${field}`);
console.log('Rust daemon v1.7 shared fixture contract passed.');
