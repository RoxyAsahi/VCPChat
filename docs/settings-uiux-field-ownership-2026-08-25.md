...
| `userName` | `userName` | 用户身份 | 用户名 / 全局显示名称 | text | typed snapshot | `save.execute`（定义级 trimValue+fallback：trim 后空值回填「用户」，对齐 legacy 收集契约） | dirty 保留；失败重试 | 通用 projection 行退役（2026-08-27，R2-02E 批次 15）；manager 兜底读保留（Classic 兼容） | Settings string owner | 已满足；presentationOwner 启动兜底已删除（2026-08-27，R2-02E 批次 20，commit 4e259486） | `typed-owner-active`; Electron trim/close-flush 证据（journey 6f） |
...
| 条件 | 证据 | 当前 |
| --- | --- | --- |
| 单一 projection owner | `check-settings-source-equivalence.mjs` + DOM inspection | 通过（2026-08-27 批次 20 后改判）：presentationOwner 启动兜底（19 个 `!typedSettingsProjectionActive` 守卫分支）已退役，全局设置表单唯一写入方 = settings-bridge typed 投影（generic projection + typed field owner）；通用 consumer 仅剩 §3 冻结 40 行 / 38 键（协议允许的责任保留） |
...
| legacy projection 删除 | source-equivalence + diff | 外观/工作区批次、enableWideChatLayout、networkNotesPaths、userName 簇已删除；启动兼容 fallback 兜底投影已删除（2026-08-27，R2-02E 批次 20/21，E1-E6 证据前置）；余量 = §3 冻结 40 行（责任保留） |
...
Home visual 扩展（2026-08-26）：`showHomeVisualBrand`、`showHomeVisualTagline` 与 `homeVisualTagline` 也已加入同一 typed field owner；通用 projection 不再重复写入，业务 key 与首页视觉行为保持不变。该扩展历史登记的 `mainChatSettingsPresentationOwner.js` 未接管 fallback 已于 2026-08-27 随启动兜底整体退役（R2-02E 批次 20，E1-E6 证据闭合）。
...
Appearance select group 扩展（2026-08-26）：`appearanceProfile.density`、`radius`、`typography`、`fontScale`、`contentWidth`、`surface` 已加入同一 typed owner，通用 projection 重复写入已删除。Appearance engine 仍是规范化与视觉应用边界；`mainChatSettingsPresentationOwner.js` 的兼容启动 fallback 已于 2026-08-27 退役（R2-02E 批次 20）。

已 complete（2026-08-27，R2-02E 批次 20）：`mainChatSettingsPresentationOwner.js` 的启动兼容 fallback 已删除（commit 4e259486），前置等价证据 E1-E6 见 handoff 补记九至十一；矩阵行 62/73 随批改判。
...
启动兼容兜底退役与死代码清扫（2026-08-27，R2-02E 批次 20/21；本档矩阵行 62/73 与 §6 三处 stale 登记随批改判）：E1-E6 证据闭合后（批次 17-19），presentationOwner 全部 19 个 `!typedSettingsProjectionActive` 守卫投影分支删除（159 行，commit 4e259486），typed 投影成为全局设置表单唯一写入方；三个落盘裁决项兑现——`speechRecognizerPagePath`/`voiceNetworkProviderUrl` 首开显示默认迁入 bridge 投影尾部（仅空值补填）、`vcpServerUrl` 接受原值显示差量（save 期 completeVcpUrl 归一照旧）、modal-ready 窗口由 typed service subscribe 回放所有。journey 1p 反转为「兜底不再瞬时填充、typed 回放同周期收复」，1q/section 6/7/8c 契约探针维持全绿。批次 21 对 presentationOwner 95 个顶层声明做引用计数扫描，删除兜底删除后失去唯一消费方的 `CHAT_PRESENTATION_MODES`/`CLASSES` 常量对与 `presentationMode` 死局部量（commit a5f1b0aa）。§3 冻结 40 行 / 38 键维持责任保留不变。
...
presentationOwner 启动兜底退役证据清单（2026-08-27，R2-02E 批次 16）：unlock 复查仍未满足（`3bc85d98` 后线程 A 零新提交，main.js / check-harness-fixture-matrix.mjs 在途改动未落盘，全部 DirectoryBrowser checkpoint 保持 `foundation-electron-active`），按矩阵存量评估下一施工候选并选定「启动兼容 fallback 退役的前置证据清单」。现状盘点：`mainChatSettingsPresentationOwner.js` 现存 **19 个 `!typedSettingsProjectionActive` 守卫分支、约 60+ 处 safeSet/safeCheck**（行 605-695 一带），覆盖全表单（typed 已迁字段 + §3 冻结字段一并兜底）；守卫条件 = 「bridge 服务已暴露 **且** `globalSettingsModal.dataset.vcpSettingsRevision` 存在」，并在 apply() 流程中两处刷新。关键定性更正：全局 `uiMode: 'classic'` 只存在于 embeddedAppSessionManager 的独立入口页拼接参数——main.html 面恒为 `uiMode: 'next'` 且 settings-bridge 无 uiMode 门控自举，因此该兜底的真实职责是**启动挂载窗口 + 部分挂载失败窗口**的控件填充安全网，而非跨页面 Classic 兼容；退役难度低于原判断，但窗口竞态证明是硬前置。退役前必须逐项产出的等价证据清单：**E1 接缝面清单**——枚举所有渲染 `#globalSettingsForm` 的入口页并证明无任一 uiMode≠next 入口加载 presentationOwner 或 settings-bridge（embedded classic 页为独立 entry，须以构建产物 grep 落证）；**E2 挂载序确定性**——冷启动旅程断言 `vcpSettingsRevision` 标记（consumer 已挂载且 project 已跑首遍）严格先于模态可打开时点，且首次打开时全部原兜底 id 值 == service snapshot（现 journey 仅覆盖子集）；**E3 部分挂载失败契约**——注入 enhance/mountSettingsShell 失败后，定义退役后的预期行为（typed readiness 门禁拒绝半挂载 shell 并保持现有 invalid-packaging/readiness 门禁语义），不得出现半填充可见表单；**E4 上游事件路由等价**——盘点驱动 apply() 的 `global-settings-updated` 全部 source（toggle 探针、appearance-studio、forum、settings-save 广播等），逐一证明在 typed subscribe→project 下守卫结果不变（可用一次性 emitLog 插桩取证，临时插桩必须按协议在提交前剥离）；**E5 reload 覆盖扩容**——矩阵行 68 已通过的 durable restore 断言集扩至全部原兜底 id；**E6 静态闭环**——source-equivalence 预期更新为「删除的 id 若重新出现第二写入方即失败」的负向守护。施工裁定：本批不删任何兜底代码（E1-E6 未产出前删除即破窗）；下一批次候选 = 按 E2/E5 扩容 journey 断言集与 E4 source 盘点（均为 B 可独立推进项），E1/E6 依赖线程 A 侧或构建产物配合度另议。docs-only：代码面自 3bc85d98 起零变更（git diff --stat 为空）。
