...
| OpenCode `pages/session/session-side-panel.tsx`、`context/layout.tsx`、command palette | **clean-room 信息架构参考** | R5-D | 浏览器式 Session Tool Dock、动态文件 Tab、工具菜单与面板折叠 | 固定 revision `a45c2b917e657e50881117e8c3f85f4bff06e47d`（MIT）；不复制 SolidJS 组件，不导入 OpenCode Runtime、Session store 或工具系统。 |
