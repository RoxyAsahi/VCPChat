...
`agent-workbench.js` is a composition root, not a destination for new message, Markdown, tool, approval, Dock, Workspace or action logic. Sidebar, Dock chrome, Notification, Approval, Topic Flow, Workspace request coordination, Timeline rendering, Session catalog, settings persistence and Topic menus now use explicit View/coordinator boundaries. The private implementation remains above its final 800-line target, so new behavior must enter the owned modules above rather than grow the composition file.
