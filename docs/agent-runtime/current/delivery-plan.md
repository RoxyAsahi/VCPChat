...
| R3-A | Cherry 式 Topic preview、同 PID attachment switch 与安全接管 | in progress | 2026-07-30，Rust source revision `7578067edb828fcccae5ca87f73b91369dc4bd02092845f64484481fd96ef469`：协议 v1.5。点击 Topic 只读 `read-topic` snapshot，不创建 Session；首次发送才以 `switch-attachment` 在同一 daemon 内安全切换 lease。`npm run build:daemon`、`npm run test:rust-agent-runtime`、`npm run test:agent-workbench-store`、`npm run test:agent-workbench`、`npm run test:rust-daemon-smoke`、`npm run test:electron-gui-smoke` 与 Rust fmt/clippy/workspace test 已通过；Electron 连续 10 个 preview Topic 保持 PID、attachment 不变并无残留 child。当前 revision 尚缺 Electron 双窗口 external lease 收据与真实 ToolBox 流，不能标记 complete。 |
...
1. 任何新建入口先显示共享 Agent、共享模型、workspace 和标题；用户确认后发送 `switch-attachment`（不含 `topicId`）创建唯一 writable Topic。
2. 点击任意持久 Topic 只执行 `read-topic` 并显示 snapshot；首次发送才 `switch-attachment(topicId)`。同 Main 的 active Turn/审批存在时预览只读，外部 lease 则在首次发送时显示“返回 / 接管并继续”冲突确认。
3. 新建表单和冲突确认只存在 Renderer 内存；Agent/model catalog 继续来自 VCPChat 共享配置，Topic、lease、snapshot 和 attachment 继续来自 daemon。
4. 2026-07-30、Rust source revision `7578067edb828fcccae5ca87f73b91369dc4bd02092845f64484481fd96ef469`：v1.5 fixture、同 PID daemon smoke、Runtime/Workbench projection、Electron 连续 10 Topic PID、close/reopen、crash/reconnect 和 child cleanup 都已通过。当前 revision 的 Electron 双窗口 external lease/接管与真实 ToolBox 收据仍待重跑，不能因 hermetic 通过而外推。
