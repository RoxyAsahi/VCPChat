...
| Electron Codex smoke | hermetic Electron | preload/IPC、Session/Thread 创建、projection-only SQLite read、Runtime identity、内部应用注册/挂载、Fork/legacy 模式与主要 DOM | working-tree pass | 2026-08-01：`npm run test:electron-codex-smoke` 通过；显式等待 `nextUiApps.get('agent-workbench')`，避免 renderer-ready 与 ES module 注册的时序误判。该命令不替代真实 Nova/ToolBox 长任务验收。 |
