...
| R11/R12 Settings/Composer | Main fake + pure model + JSDOM + Electron + live ToolBox | Profile/Session/Advanced 作用域、指令模式、CAS、desired/applied、真实配置 barrier、A/B 草稿附件模式隔离、steer/follow-up/stop | implemented / live settings pass | R12 专项与 `test:electron-codex-smoke` 通过；真实 UI 已验证 YOLO/cwd 保存且控件不回跳。完整 Main 进程重启恢复已通过。2026-08-03 `test-codex-settings-live.mjs` 验证真实 `thread/settings/update` 与下一 Turn provider body 的 model/cwd/approval/effort；指令模式切换仍 pending。 |
