...
| AG-R24 | P2 | Archived Pi/Rust tests are mistaken for supported product code | Archive-only npm names, package exclusion, product import scan | `check:agent-runtime`, `check:codex-governance`, `archive:pi:test-runtime` |
...
| AG-R31 | P2 | Canonical Agent JavaScript accumulates syntax hazards or silently exceeds the current complexity baseline | ESLint correctness rules, complexity ceiling 170, mandatory `lint:agent` in `test:codex-ci`, import-graph cycle check | `npm run lint:agent`, `check:codex-governance`, `test:codex-ci` |
