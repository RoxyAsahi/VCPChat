# Agent Runtime 路线图（Pi 阶段历史计划）

> 2026-07-29 状态：本路线图以 Pi Worker 为中心，已不代表当前 GUI 的实施顺序。Rust daemon/GUI 的真实缺口、阻塞项和收敛顺序见 [GUI 当前开发状态](../gui-current-development-status.md) 与 [Rust daemon 与 Agent GUI 收敛决策](gui-daemon-integration.md)；本文仅保留为阶段 0–5 的历史决策记录。

## 当前收敛路线（Rust GUI，优先于下列 Pi 历史阶段）

1. **唯一 Runtime 真源**：仓库内 `rust/` 是正式源码；禁止与独立 VCPAgent worktree 并行修改同一产品能力。Pi/mock 仅 legacy 测试。
2. **收缩 GUI 接入**：Electron Main 只监督 daemon；`RustAgentRuntimeManager` 不拥有第二份 transcript/Topic/Turn 真源；Workbench 只消费 daemon snapshot 与事件。
3. **稳定投影**：以 `sessionId + turnId + toolCallId` 和 daemon message key 构建 event-to-block reducer；消除基于 DOM、数组位置或 localStorage transcript 的恢复逻辑。
4. **先验收再扩展**：每次协议、Topic 或 reducer 变更必须跑 `npm run build:daemon` 与 hermetic Electron E2E；ToolBox 真实调用只经显式 live gate。未完成前不增加新的 Pi Driver、Catalog、Patch、Subagent 或 Team 产品入口。
5. **产品页验收**：完成 Agent Workbench 的新建/恢复/刷新/重连/审批/工具卡/取消/压缩视觉与交互回归后，才继续经典 UI 与 Next UI 的更大范围迁移。
