...
信任边界与数据流方向详见 [security-threat-model.md](security-threat-model.md#信任边界)。Worker 内 Pi 的**全部**内置工具（read/write/edit/bash）与任意 extension 自动加载**一律禁用**；Worker 中唯一允许触达外部世界的能力是经 VCP 工具桥发往 VCPToolBox 的 HTTP 调用（AR-SEC-008）。
...
renderer (agent-workbench)
   ...
   → main: modules/agent-runtime/ (Manager / ApprovalBroker / EventNormalizer)
   → main: modules/agent-runtime/drivers/* (AgentRuntimeDriver facade)
   → worker: agent-runtime/ (Pi adapter + VCP 工具桥)
...
- Worker **不得**直接弹窗、读写用户文件或执行 shell；这些诉求只能转化为工具调用走桥。
...
3. Manager 调用 `driver.startSession()`；Worker 侧初始化 AgentHarness，禁用内置工具与 extension，注入 VCP 工具桥工具集。
...
3. 本地批准后，Worker 经 VCP 工具桥发起 `vcp_invoke`/`vcp_delegate`；**ToolBox 后端审批（VCPLog WebSocket 往返）照常执行，不被本地批准跳过**（AR-SEC-007）。
