...
| Agent model picker / `AgentModelPicker` | Harness `ui-model-selection` picker contract; VCP model capability must be injected | Candidate Lab-only `mountAgentModelPicker` over owner-bound `PopupSelect` | no VCP production consumer; existing `modelSelectModal` remains canonical and untouched | add real Harness model-selection DOM/geometry/pixel fixture, then evaluate a reversible Settings consumer without changing IPC/persistence | `modules/uiux/primitives/agent-model-picker.ts`, `tests/uiux-primitives.test.mjs`, `scripts/capture-vcp-agent-model-picker-candidate.mjs`; 800x600 evidence proves root/model/effort panes, 240px in-viewport menu, ArrowDown, Escape, focus restore and dispose | `candidate-interaction-active / source-equivalence-pending`; not `productionEligible` |
...
| Agent action controls / `openModelSelectBtn`, submit, `deleteAgentBtn`, `refreshTtsModelsBtn` | existing Agent Settings button handlers, `SettingsActionBar` state machine and TTS refresh capability | `settings-bridge` presentation scope + generated Light-DOM Button; native buttons and action-bar remain canonical | Button owns class/style/dispose only; click handlers, dirty/saving/deleting/error states, model modal and TTS refresh remain legacy/business-owned | add per-button default/disabled/busy/focus/reload/teardown and same-engine geometry/pixel evidence, then remove only field/button-specific legacy selectors after action-bar behavior remains covered | `modules/ui-system/settings-bridge.js`, `modules/ui-system/vcp-ui.js`, `scripts/check-agent-settings-production-evidence.mjs`; current production DOM gate verifies canonical ids and Harness classes | `production-consumer-active / visual-equivalence-pending`; not `verified-candidate` |
