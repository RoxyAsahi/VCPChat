...
| Directory flow | `ui-directory-picker-native/`, `ui-directory-picker-browse/` | foundation-electron-active：Light-DOM Miller browser 已使用 injected listing/create/open/close capabilities，覆盖 single/two pane、hidden entries、abort/close/reopen/dispose，并有 800×600 Electron fixture；breadcrumb → submission-only path editor、nested create dialog（parent inert、Escape retract、创建后选中）和 300ms slow-scan gate 已有 artifact/Electron 证据。绝不接入 VCP directory IPC、Workspace persistence 或 dynamic-wallpaper path selection。Harness 的 draft preview/filter、pixel diff 与 VCP production consumer pending | B2 |
