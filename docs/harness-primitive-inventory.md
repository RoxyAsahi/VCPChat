...
| Agent model picker / `AgentModelPicker` | Harness `ui-model-selection` picker contract; VCP model capability must be injected | Candidate Lab-only `mountAgentModelPicker` over owner-bound `PopupSelect` | no VCP production consumer; existing `modelSelectModal` remains canonical and untouched | add real Harness model-selection DOM/geometry/pixel fixture, then evaluate a reversible Settings consumer without changing IPC/persistence | `modules/uiux/primitives/agent-model-picker.ts`, `tests/uiux-primitives.test.mjs`; no production consumer | `candidate-interaction-active / source-equivalence-pending`; not `productionEligible` |
