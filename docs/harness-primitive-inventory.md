...
| Directory flow | `ui-directory-picker-native/`, `ui-directory-picker-browse/` | foundation-electron-active：Light-DOM Miller browser 已使用 injected listing/create/open/close capabilities，覆盖 single/two pane、hidden entries、abort/close/reopen/dispose，并有 800×600 Electron fixture；绝不接入 VCP directory IPC、Workspace persistence 或 dynamic-wallpaper path selection。path editor、nested create dialog、slow scan 与 pixel diff pending | B2 |
