...
| Agent model picker / `AgentModelPicker` | Harness `ui-model-selection` picker contract; VCP model capability must be injected | Candidate Lab-only `mountAgentModelPicker` over owner-bound `PopupSelect` | no VCP production consumer; existing `modelSelectModal` remains canonical and untouched | complete same-engine DOM/computed-style/pixel capture, then evaluate a reversible Settings consumer without changing IPC/persistence | `modules/uiux/primitives/agent-model-picker.ts`, `tests/uiux-primitives.test.mjs`, `scripts/capture-vcp-agent-model-picker-candidate.mjs`, `docs/reference/deepseek-harness-primitives/model-picker.{dom,geometry,css}`; geometry contract records 28px trigger, 240px menu, 40px root cells and 38px options | `candidate-interaction-active / visual-equivalence-pending`; not `productionEligible` |
