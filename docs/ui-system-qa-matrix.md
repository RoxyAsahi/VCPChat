...
| Web Awesome 适配器 | `WebAwesomeAdapter` 契约测试全过（属性/事件/`updateComplete`/refcount 主题/销毁/next 守卫）；业务目录门禁 `check:ui-applications` 生效（禁 `<wa-*>`、裸 `--wa-*`、CDN、未登记 `::part()`） | — |
...
| 全局设置 | 真实 Electron 验证：next 模式下 `#globalSettingsForm` 控件增强、保存栏 SettingsActionBar 脏/保存态、搜索注入、切经典清理 | 7 个分区逐项真实保存/错误/键盘 Escape |
...
| 迁移页面运行时 | 12 页已真正重建（AppPageShell + VCPUI 控件 + VCPUI Tooltip；便签/翻译/日志经真实 Electron embedded 逐页验证）；门禁强制「业务页仅经 VCPUI、禁 VCPWebAwesome/`<wa-*>`」 | 全页 embedded/standalone 行为回归扩展；全局设置结构重构；骰子 |
