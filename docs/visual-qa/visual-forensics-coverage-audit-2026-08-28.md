...
| --- | --- | --- |
| Fixed `800x600`, `1280x800`, `1680x1000` viewports in light and dark | Fresh isolated Showcase pair `reports/visual-forensics-qa/run-ZQDdcv/{light,dark}`, three observations each; `check-visual-forensics-evidence` and `check-visual-forensics-baseline` pass for all six combinations. | Passed |
...
| Global Settings primitive controls | Fresh real Electron isolated pair `reports/visual-forensics-qa/global-settings-controls/run-ggGwn8/{light,dark}` passes at `800x600`, `1280x800`, and `1680x1000`: visible Input/Select/Range/Toggle/Choice counts, per-control finite geometry and computed-style samples, Select portal in-viewport/topmost geometry with hover/focus, range projection without layout drift, scroll-owner movement, close/reopen with distinct root identity, and no horizontal overflow. Wrapper `scripts/run-visual-qa-next-global-settings-controls-themes.mjs` now runs both themes and the dedicated verifier together. | Passed |
...
| Typed Settings Range in production context | The current real-Electron Settings gate mounts native Range wrappers for `#agentTtsSpeed`, `#appearanceSidebarRowHeight`, `#appearanceSidebarAvatarSize`, and `#appearanceCustomRadius`; it verifies output projection, close-flush persistence, reload/reopen, and explicit presentation-scope teardown. The concurrent fixed-viewport scanner `run-10UtiP/{light,dark}` records the real Settings modal, geometry, computed style, DOM, and cascade in both themes at all three required sizes. | Passed |
...
| Production Agent Settings ModelPicker | Fresh real Electron pair `reports/visual-forensics-qa/agent-model-picker/run-YTgOBc/{light,dark}` passes the independent verifier at all three viewports after the portal safe-area fix. The bounded runner exits normally after writing both complete `gate.pass:true` manifests; the verifier now rejects screenshots stale by more than 120 seconds relative to each manifest. | Passed |
| Production ModelPicker hover / resize / Escape | Each `run-YTgOBc` capture records an enabled `role=option` with visible `:hover`, finite viewport rect, card-contained hit test, fixed/body portal after narrow and restoration, and Escape focus/body cleanup. | Passed |
| Production ModelPicker SemanticIcon slot | Fresh pair `reports/visual-forensics-qa/agent-model-picker/run-YTgOBc/{light,dark}` proves the shipped Agent Settings trigger renders its generated SemanticIcon slot in real Electron at all three viewports: a `14×14` SVG, `viewBox="0 0 24 24"`, `aria-hidden="true"`, and a visible path. The production ModelPicker portal, transient actions, hover, narrow/restored resize, Escape focus restoration, and body cleanup verifier also passes. This is consumer evidence only; it does not claim Harness filled-glyph or pixel parity. | Passed for consumer presence and runtime contract |
...
| Freshness after dirty worktree UI changes | Current dirty snapshot was captured in isolated pair `reports/visual-forensics-qa/run-ZQDdcv/{light,dark}`. Both manifests have three observations and `gate.pass:true`; exact-pair evidence, token contrast, and all six pixel/geometry baseline checks pass, including native-pointer Tooltip activation after sticky-header-safe scrolling. | Passed for this snapshot; rerun after subsequent UI edits settle |
...
```text
reports/visual-forensics-qa/agent-model-picker/run-YTgOBc/light
reports/visual-forensics-qa/agent-model-picker/run-YTgOBc/dark
```
