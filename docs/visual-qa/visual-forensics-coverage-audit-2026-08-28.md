...
| Global Settings primitive controls | Fresh real Electron captures `reports/visual-forensics-qa/global-settings-controls/diagnostic-{light,dark}` pass at `800x600`, `1280x800`, and `1680x1000`: visible Input/Select/Range/Toggle/Choice counts, per-control finite geometry and computed-style samples, Select portal in-viewport/topmost geometry with hover/focus, range projection without layout drift, scroll-owner movement, close/reopen with distinct root identity, and no horizontal overflow. Dedicated verifier `scripts/check-visual-qa-next-global-settings-controls.mjs` passes both reports; this remains a separately invoked Settings gate. | Passed |
