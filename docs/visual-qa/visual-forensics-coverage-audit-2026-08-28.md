...
| Production Agent Settings ModelPicker | Fresh real Electron captures pass the independent verifier at all three viewports after the portal safe-area fix. The runner teardown is now bounded; `agent-model-picker/diagnostic-teardown-v2-light` exits 0 after writing a complete `gate.pass:true` manifest. | Passed |
