...
| Global Settings primitive controls | Fresh real Electron captures `reports/visual-forensics-qa/global-settings-controls/diagnostic-{light,dark}` pass at `800x600`, `1280x800`, and `1680x1000`: visible Input/Select/Range/Toggle/Choice counts, Select portal in-viewport/topmost geometry with hover/focus, range projection without layout drift, scroll-owner movement, close/reopen with distinct root identity, and no horizontal overflow. | Passed for this capture; add a dedicated verifier before promoting to the default npm gate |
