...
`2026-08-28T03:26:36.461Z` (dark). The later paired refresh is
`reports/visual-forensics-qa/agent-model-picker/run-YTgOBc/{light,dark}`.

## Triage

| Priority | Finding | Electron evidence | Minimal correction / gate | Current state |
| --- | --- | --- | --- | --- |
| P1 | Production ModelPicker card could resolve at `x=-4` and its center could hit the Settings header at `800x600`; this was a real clipping/stacking failure. | Earlier production probe recorded the off-screen card and foreign center hit. The refreshed pair `run-YTgOBc` records a `230x50` fixed card in-viewport and topmost at every required viewport, including narrow/restored states. | Portal the card with fixed geometry; preserve the existing native trigger. Dedicated production gate checks fixed/body/geometry/hit-test, Escape, focus restoration, and body inline-style cleanup. | Resolved; fresh paired evidence verified. |
