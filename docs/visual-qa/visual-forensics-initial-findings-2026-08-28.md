...
| P2 (open) | Same-engine paired ModelPicker ROI comparison remains approximately `6.19%` different pixels. | Existing comparison artifact records the mismatch; the production interaction reports prove portal/lifecycle correctness but do not prove pixel equivalence. | Do not mark the primitive Stable or retire the legacy modal. Add strict paired ROI comparison after product parity scope is agreed. | Open; intentionally excluded from the interaction/lifecycle pass claim. |
