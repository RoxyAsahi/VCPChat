...
已完成：建立 `08511fa5` 稳定基线，恢复设置页行布局、字号/数值步进、字体选择、自动保存兼容和 portal 层级；共同祖先到 `origin/main` 的 96 个提交已逐项登记，业务整域按上游文件树收口，共享设置文件只保留 presentation 适配。Select 的 MiMo 音色 `<optgroup>` 分组投影与 remount 时序回归也已补齐。
