...
completed_slice: semantic token projection + scope-owned SettingsUiService registry assembly + appearance/font/streaming control projections with Electron evidence
...
当前 SettingsRoot 状态（2026-08-24）：全局 Settings form submit 已通过 `VCPUISettingsBridge.getTypedService().save.execute()` 进入 typed command owner；`SettingsUiService` 现由 bridge-owned child scope 内的局部 `UiServiceRegistry` 安装，生产 consumer 通过 `get('settings-ui')` 获取，显式 release、owner teardown、逆序 dispose、provider failure rollback 与 generated artifact smoke 已有 focused 证据。该 registry 只用于 UI service assembly，不接入 Plugin Loader、chat manifest 或全局插件协议。clean external snapshots 现在真实驱动用户身份、服务器、摘要、Home、Appearance（density/radius/typography/fontScale/contentWidth/surface/sidebar geometry）、字体 preset/custom、聊天气泡和 smooth streaming 控件；checkbox 使用 `checked` 投影，dirty/in-flight 时仍拒绝外部覆盖。`node scripts/test-settings-wa-electron.mjs` 已覆盖这些控件的 clean projection，并继续覆盖失败保存、重试、重载恢复和 owner teardown。其余读取投影、dirty/autosave 编排仍主要由 legacy `settings-bridge.js` 与 global settings manager 控制，因此 R2-02 整体仍是 typed-command-active 而非完整 Settings consumer。
