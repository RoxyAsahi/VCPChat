...
当前 SettingsRoot 状态（2026-08-24）：全局 Settings form submit 已通过 `VCPUISettingsBridge.getTypedService().save.execute()` 进入 typed command owner；`userName` 与 `continueWritingPrompt` 两个高频字段现在通过 `SettingsUiService.state` 消费 clean external snapshots，并在 dirty/in-flight 时拒绝外部覆盖。字段/Select/Choice 的其余读取投影、dirty/autosave 编排仍主要由 legacy `settings-bridge.js` 与 global settings manager 控制，因此整体仍是 typed-command-active 而非完整 Settings consumer。adapter 已具备保存串行化、迟到 generation、dispose、subscriber rollback/isolation 和失败事件合同，Electron journey 已覆盖 clean external projection、dirty draft protection、真实持久化失败、输入保留、点击重试成功、重载恢复、结构/截图及 bridge/presentation teardown。下一步继续迁移剩余高频字段并减少旧 bridge 控制面，在完成行为等价证据后才删除旧 presentation 路径。
