...
VCP Select fixture validation（2026-08-26）：证据 gate 现在要求 VCP Select report 的 DOM 必须包含 `.vcp-harness-menu-list`，并拒绝错误捕获的 legacy sidebar menu。当前历史 `reports/vcp-select-production.json` 不满足该条件，状态为 `pending-invalid-or-missing`；需重新执行 generated-artifact Electron Select-only capture 后才能合并 semantic route。
状态修正（2026-08-26）：上方早期“Select browser fixture pending”的历史记录已被本次 production capture supersede；当前 Harness Select fixture 为 `available`，未闭合的是 Harness Select-only 与 VCP Input+Select 的 semantic route 合并，以及同场景 pixel diff。
