...
4. 只实现一个真实 Light-DOM `Field + Select` primitive，接入 SettingsRoot 的 `appearanceDensity`；`node scripts/test-settings-wa-electron.mjs` 已锁定 Field/Select DOM、40/8/10 geometry、菜单交互及截图路径。下一步是补充独立 interaction sequence 与 source/artifact 双平面证据，再删除该字段 legacy projection。
