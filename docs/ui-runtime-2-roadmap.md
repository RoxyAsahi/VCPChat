...
next_slice: Field description/error visual slice is closed. Harness `ui-primitives/Input` currently has no production consumer, so its source-derived fixture is not eligible to unblock the pipeline. Agent Preset Select closed-trigger raw capture is now available and proves the generic VCP Settings Select is not the same trigger contract. The keyboard-open menu has a separate focus-owner mismatch: Harness retains trigger focus, while VCP transfers it to the selected menuitem and supports roving navigation. Preserve this report until an explicit interaction-contract decision; do not claim visual ROI equivalence as keyboard equivalence. Do not define an `AgentPresetSeat` provider until a real, in-scope VCP product consumer is identified: current VCP `assistantAgent` is still legacy-owned and field migration is frozen, while chat-side assistant switching is a frozen boundary. The disabled trigger may enter this batch only after a reproducible production `state.busy` route exists; a disabled Menu item remains source-contract evidence, not Agent Preset production evidence.
blocked_by: explicit focus-ownership interaction-contract decision; a real, in-scope VCP consumer for the AgentPresetSeat or AgentPresetRow trigger contract; reproducible busy-trigger route before disabled-trigger production closure; dynamic-wallpaper disabled-manifest UI Apps smoke is external and nonblocking for this slice
...
Select keyboard-focus evidence（2026-08-26）：`capture:harness-select-focus-fixture` 与 `capture:vcp-select-focus-browser-fixture` 以相同 `AgentPresetSeat → Standard mode → trigger focus → Enter` 路径捕获 `document.activeElement` 的语义 owner；`diff:harness-vcp-select-focus` 单独输出 `reports/harness-vcp-focus-geometry-diff.json` 和 focus pixel report，绝不覆盖 hover 的通过报告。真实 Harness 键盘打开后保留 trigger 焦点；VCP `mountSelect()` 则把焦点转移到 selected menuitem 并启用 roving arrow navigation。因此此状态必须稳定报告 `cross-page-select-focus-owner-mismatch` / `pass=false`，不得将 hover ROI pixel pass 扩展为 keyboard interaction equivalence。后续是否迁移 VCP 的 focus ownership 是一个独立产品/无障碍合同决策；在决定前冻结新的字段迁移、renderer 扩展和 primitive 扩展。
