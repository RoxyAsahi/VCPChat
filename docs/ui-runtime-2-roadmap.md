...
completed_slice: semantic token projection + scope-owned SettingsUiService registry assembly + appearance/font/streaming/base-text/VCP connection/voice/advanced/middle-click/chat-layout/assistant projections, combo-control synchronization, legacy projection gating, and Electron evidence
...
当前 SettingsRoot 状态（2026-08-24）：全局 Settings form submit 已通过 `VCPUISettingsBridge.getTypedService().save.execute()` 进入 typed command owner；`SettingsUiService` 现由 bridge-owned child scope 内的局部 `UiServiceRegistry` 安装，生产 consumer 通过 `get('settings-ui')` 获取，显式 release、owner teardown、逆序 dispose、provider failure rollback 与 generated artifact smoke 已有 focused 证据。该 registry 只用于 UI service assembly，不接入 Plugin Loader、chat manifest 或全局插件协议。clean external snapshots 现在真实驱动用户身份、assistantAgent 动态 select、服务器 URL、VCP API/File/Log 连接字段、语音 mode/识别器路径/本地与网络 provider、分布式服务/工具注入/思维链/上下文净化/AI 按钮/音乐控制 checkbox、净化深度与可见性、flowlock/middle-click/regenerate controls、chat presentation/layout mode、用户气泡宽度与 stream tuning 及其可见性、摘要模型、Home、Appearance（density/radius/typography/fontScale/contentWidth/surface/sidebar geometry）、字体 preset/custom、聊天气泡和 smooth streaming 控件；checkbox/radio 使用 `checked` 投影，动态 assistant options 通过 MutationObserver 重放 snapshot，sidebar radius radio group、range output（px）与 hidden compatibility select 同步，dirty/in-flight 时仍拒绝外部覆盖。`mainChatSettingsPresentationOwner` 已在 typed service 可用时停止覆盖这些迁移字段，保留 Rust Assistant 独立 capability（`getRustAssistantConfig` / `saveRustAssistantConfig`）不纳入 global Settings adapter；`node scripts/test-settings-wa-electron.mjs` 已覆盖 clean projection、失败保存、重试、重载恢复和 owner teardown。其余读取投影、dirty/autosave 编排仍主要由 legacy `settings-bridge.js` 与 global settings manager 控制，因此 R2-02 整体仍是 typed-command-active 而非完整 Settings consumer。
