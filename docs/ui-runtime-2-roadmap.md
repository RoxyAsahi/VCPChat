...
2026-08-26：Forum `adminUsername` / `adminPassword` 已进入 presentation-only 阶段：由 TS Light-DOM Input primitive 提供 32px/8px/8px geometry 与 scope-owned teardown，native inputs 和既有 ForumConfigUiService/global submit 仍是业务与 command source。Electron Settings gate 新增两项 primitive DOM 断言并通过；该字段仍标记为 `typed-input-presentation-active`，未宣称 single-owner，未删除 dirty/autosave legacy path。
