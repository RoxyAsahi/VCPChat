...
> 最近核验：2026-08-24；R2-00 Composer slice 已达到 complete；R2-01 Overlay/notification slice 已闭合；R2-03 ThemeSnapshot adapter 已接入；当前 active slice：R2-08 TypeScript foundation  
...
| U1 TypeScript UI foundation | R2-08 `modules/uiux/` typed kernel | planned |
...
| U4 Settings Surface | R2-02 | planned |
...
batch: R2-08
mode: target
focus: typescript-ui-foundation
status: in_progress
production_consumer: typed contract seam and LifecycleScope adapter (candidate; not stable API evidence)
consumer_kind: foundation candidate; real Surface consumer required before completion
first_slice: modules/uiux/contracts.ts + modules/uiux/runtime/scope.ts + providers/theme.ts
next_slice: typed ThemeUiDefinition adapter consumed by AccountMenu or Overlay Surface in a built Electron artifact
...
evidence: npm run check:uiux; npm run test:uiux; node --test tests/state-authority.test.js tests/account-menu-controller.test.js tests/ui-manager-lifecycle.test.mjs
```
...
1. 完成 R2-03：把 Appearance Studio 的读/提交路径接到唯一 ThemeSnapshot，并补 light/dark/system、壁纸和 reload 的真实证据。
2. 在进入新的公共 UI primitive 前先施工 R2-08 TypeScript foundation：`UiScope`、`UiEffect`、typed service definition/provider/consumer、contract gate；不复制现有 `LifecycleScope`，而是提供兼容 adapter。
3. 保持 R2-00/R2-01 的 Slot、Overlay、Focus 能力为真实 consumer 驱动的 internal API，不开放任意 selector/HTML 注入。
4. UI Apps smoke 当前明确失败于 `loaded=true, results=[], dynamicWallpaper=false`；根因是 `VChatDynamicWallpaper/plugin-manifest.json.block`。不擅自启用用户禁用插件，先保留为外部 readiness blocker。
