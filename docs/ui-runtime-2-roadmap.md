...
> **2026-08-28 Visual Forensics / ModelSelect capture checkpoint：** Visual Forensics light/dark Electron evidence 已在 `800×600`、`1280×800`、`1680×1000` 下重新生成并通过 `npm run check:visual-forensics`，覆盖 hover/focus、overlay、Settings context、close/reopen 与 CDP cascade。新增 Harness `ModelSelect` production capture 已通过真实 scaffold 采集 root/model DOM、ARIA、computed-style、geometry 与截图；`check:harness-model-picker-reference` 为 `26/26`。VCP 对照当前明确为 `harness-capture-available-pixel-pending` / `pixel-dimension-mismatch`，因为两端仍不是同一 ROI；Effort、Escape、focus 的 Harness capture 仍记录为缺口，不能晋级 `verified-candidate` 或 `stable`。
