...
2026-08-26 state fixture expansion：Harness 真实组件与 VCP generated artifact 现已各自捕获 Input default/focus/disabled、Select closed/open/selected/disabled；Field description 仅已有 VCP fixture，Field error 与 Harness Field fixture 仍缺。矩阵状态更新为 `state-fixtures-partial-captured`，不宣称九状态完整。
2026-08-26 state structural diff：`diff:harness-vcp-dom` 扩展到 7 个已捕获状态，结果 5/7 structurally equal；Input 三态与 Select open/selected 等已对齐，Select closed/disabled 的菜单存在性与 disabled 属性仍暴露差异。报告保持 `pass=false`，继续作为修正依据。
