...
> **2026-08-28 Visual Forensics / ModelSelect capture checkpoint：** Visual Forensics light/dark Electron evidence 已在 `800×600`、`1280×800`、`1680×1000` 下重新生成并通过 `npm run check:visual-forensics`，覆盖 hover/focus、overlay、Settings context、close/reopen 与 CDP cascade。Harness `ModelSelect` production capture 已通过真实 scaffold 采集 root/model/effort DOM、ARIA、computed-style、geometry、Escape pane-back、root close 与 trigger focus restore；`check:harness-model-picker-reference` 为 `26/26`。VCP 对照当前明确为 `pending-semantic-fixture-alignment`，因为 Harness 是 2 个 provider-grouped `menuitemradio` 且无 search，VCP Candidate 是 3 个增强选项、search 与 generic `role=option`；不能晋级 `verified-candidate` 或 `stable`。
...
> **2026-08-27 Agent model trigger Button：** `agentSettingsForm #openModelSelectBtn` 已接入生成的 Harness Button primitive（outline/sm），仅接管 Light-DOM class/style 与 owner dispose；点击处理、旧 `modelSelectModal`、IPC、持久化和自由文本 `#agentModel` 均保持 canonical。该按钮状态为 `production-consumer-active / visual-equivalence-pending`，不等同于模型 picker production 接入。
> **2026-08-27 Agent model-picker production slice：** `agentSettingsForm #openModelSelectBtn` 现在由生成的 `AgentModelPicker` 真实接管展示和交互；它复用原生按钮节点，异步读取 `chatAPI` 缓存模型并把选择写回唯一 canonical `#agentModel`，保留原有 `input/change`、`saveAgentConfig`、IPC 与持久化语义。Electron 证据已覆盖 open → root → model → search/filter → select → close → reopen → Escape → focus restore，独立命令为 `npm run test:electron-agent-model-picker`。legacy `modelSelectModal` 的热门/收藏/显式刷新能力仍未退役；同引擎 Harness DOM/computed-style/geometry/pixel、reload/teardown 与 legacy deletion 证据仍 pending，因此状态为 `production-consumer-active / visual-equivalence-pending`，不是 Stable 或完整 ModelSelect 等价。
> **2026-08-27 Agent action Button：** `agentSettingsForm` 的保存提交按钮与删除按钮现接入同一生成 Button primitive；`SettingsActionBar` 仍负责 dirty/saving/deleting/error 编排，按钮只负责 presentation 与 dispose。两者状态为 `production-consumer-active / visual-equivalence-pending`，不退役 action-bar 或删除 legacy 状态投影。
...
> **2026-08-27 下一施工游标：** Agent 设置基础 Input 与模型 picker 已有真实 production consumer；暂停继续扩大字段迁移，转为闭合 ModelPicker 的 Harness 对照证据和 legacy modal 退役条件。优先补同引擎 DOM structural、contract-scoped computed-style/geometry、固定 viewport screenshot/pixel、reload/teardown 与热门/收藏/刷新能力 parity；在这些证据闭合前不扩展 DiffBlock、Directory、通用 renderer 或冻结聊天内容。
...
> **2026-08-27 ModelSelect reference pack：** 新增 Harness `ui-model-selection/ModelSelect.tsx` 的 DOM/CSS reference 与 fixture matrix 条目，明确 root/model/effort 三层 pane、trigger/menu ARIA、loading/error-retry/selecting/locked 状态。当前仅完成源码 provenance 与候选状态登记；VCP Candidate 与 Harness 的同引擎 DOM/computed-style/geometry/pixel 对照仍 pending。
...
> **2026-08-27 ModelSelect Candidate evidence refresh（历史记录，已被后续 production slice supersede）：** 固定 `800×600@1x` Electron capture 验证了 `ArrowDown` active option、model/effort 两层 Escape 返回 root、关闭后 trigger focus restore，以及 menu `240px` 宽度与 viewport 内 `getBoundingClientRect`。随后该 Candidate 已接入 `agentSettingsForm`；当前 production 状态仍为 `production-consumer-active / visual-equivalence-pending`。
...
> **2026-08-27 ModelSelect ARIA contract（历史记录）：** Candidate menu 使用稳定 id 与 `role="menu"`，trigger 通过 `aria-controls` 绑定 menu，并提供非空 `aria-label`；fixed Electron capture/gate 已覆盖该关系。后续 production slice 复用该合同，但 legacy `modelSelectModal` 的热门/收藏/刷新能力仍未退役。
...
R2-02E presentationOwner 兜底退役证据 E2/E5/E4（2026-08-27，批次 17）：unlock 复查仍未满足（线程 A 已推进 DirectoryBrowser 七个 checkpoint，但状态仍为 `foundation-electron-active`，fixture-matrix 门禁亦锁定 pending 声明），按预案推进 E2/E5/E4。E2：journey 新增 1p——实测 SettingsRoot 在首次 openModal 周期内挂载、冷启动时 revision 不存在，「先于可打开」假设被证据更正为「revision 与表单同打开周期就绪」——与 1q 首开 oracle（40 个兜底 id + 路径行全量镜像快照，零 mismatch）。E5：external 事件补 topicSummaryModel/字体簇 6 键，reload restore 断言扩至全部兜底 id。E4：全仓盘点 `global-settings-updated` 共 4 emitter（settings-load/settings-save/appearance-studio×2）+ 4 listener（typed service/select mirror/home visual/account menu），presentationOwner 完全不监听该事件（event-deaf），唯一存活触发面为 modal-open 同步与冷启动 loadAndApply，`renderer.js` 的 syncGlobalSettingsToUI 包装已无外部调用方——删除兜底不触碰事件路由图。退役差量落盘待决：speechRecognizerPagePath 与 voiceNetworkProviderUrl 的首开显示默认值仅经兜底上屏（renderer.js 默认宇宙），typed state 为原始持久化数据；vcpServerUrl 的 save 期 completeVcpUrl 补全仅存在于 legacy collect。6d 既有怪癖（diary select 无 monospace 选项、空串草稿、reload 重放 `|| 'serif'`）一并记录。门禁四快 + journey 全轮 PASS；兜底代码零删除。
R2-02E 兜底退役证据 E1/E6（2026-08-27，批次 18）：unlock 复查仍未满足（DirectoryBrowser 保持 `foundation-electron-active`）。E1 落源码级断言：main.html 为唯一渲染 `#globalSettingsForm` 的文档（静态 `data-ui-mode="next"`），embeddedAppSessionManager allowlist 的 9 个 classic 内嵌页零 settings 面引用，兜底职责域锁死在 main.html 启动/半挂载窗口。E6 负向守护落 source-equivalence：39 个兜底 id 逐一钉死「控件存在 + presentationOwner 持有 + bridge 覆盖 + 外部触碰文件集合 deepEqual」，新第二写入方即门禁失败；现存量枚举 legacy 整表收集/blur 补全/颜色 picker/appearance-studio/renderer 接线五类。E1-E6 仅剩 E3；门禁 exit 0，兜底代码零删除。
R2-02E 兜底退役证据 E3 闭合（2026-08-27，批次 19）：契约定性更正——兜底同步唯一触发面为一次性 `modal-ready`（模板首次实例化）+ 冷启动 loadAndApply，reopen 永不重跑；`modal-ready` 同步先于 MutationObserver 微任务，首开填充窗口由兜底所有是结构性事实，typed 投影同周期收复（1p modal-ready 瞬时探针）。8c 负向契约：ready 标记删除后 reopen 草稿逐字保留；服务层对等值外部通知去重，真实状态变更才重落 revision 并重投影。E1-E6 全部闭合，兜底退役具备全部前置证据，剩余裁决项（显示默认归属/completeVcpUrl 差量/modal-ready 替代 owner）落盘 handoff。journey 全轮 + 四快门禁全绿；兜底代码零删除。
R2-02E 启动兼容兜底退役施工（2026-08-27，批次 20）：unlock 复查仍未满足（DirectoryBrowser 保持 `foundation-electron-active`），按批次 19 闭合的 E1-E6 前置证据执行删除施工。`mainChatSettingsPresentationOwner.js` 全部 19 处 `!typedSettingsProjectionActive` 守卫投影分支删除（159 行），typed 投影（bridge generic projection + typed field owner）成为全局设置表单唯一写入方；presentationOwner 仅保留控制绑定/实时预览/辅助职责。三个裁决项兑现：`speechRecognizerPagePath` 与 `voiceNetworkProviderUrl` 首开显示默认迁入 bridge 投影尾部（仅空值补填，typed state 保持原始持久化语义）；`vcpServerUrl` 接受原值显示差量（save 期 completeVcpUrl 归一照旧）；modal-ready 窗口由 typed service subscribe 回放所有（immediate 默认回放已证）。journey 1p 反转为「兜底不再在 modal-ready 瞬时填充、typed 回放同周期收复」，1q/section 6/7/8c 契约探针全部维持；E6 门禁反转为退役断言（presentationOwner 禁止复活守卫与投影分支、bridge 必须持有迁入默认、FALLBACK_TOUCHERS 循环改「控件存在 + bridge 覆盖 + 外部触碰集合 deepEqual」三件套）。ledger §6「启动兼容 fallback 尚未删除」关闭。四快门禁 + Electron journey 全轮 PASS。
R2-02E 兜底退役后死代码清扫（2026-08-27，批次 21）：unlock 复查仍未满足（DirectoryBrowser 保持 `foundation-electron-active`）。对 presentationOwner 95 个顶层声明做引用计数扫描，删除兜底投影分支删除后失去唯一消费方的两处死代码——`CHAT_PRESENTATION_MODES`/`CHAT_PRESENTATION_MODE_CLASSES` 常量对与挂载体内 `presentationMode` 死局部量（6 行）；`normalizeChatPresentationMode`（radio 同步器）、`safeSet`/`safeCheck`（rust 回退）、`syncChatFontControls`（实时预览监听）确认存活保留。零行为变更，四快门禁 + Electron journey 全轮 PASS。
R2-02E 归属矩阵台账对账（2026-08-27，批次 22，docs-only）：unlock 复查仍未满足（DirectoryBrowser 保持 `foundation-electron-active`）。批次 20/21 删除启动兜底后，对 `docs/settings-uiux-field-ownership-2026-08-25.md` 六处 stale「当前」状态对账——§2 userName 行兜底惰性表述改为已删除；矩阵行 62「单一 projection owner」从「部分通过」改判「通过」（唯一写入方 = settings-bridge typed 投影，§3 冻结 40 行为协议允许责任保留）；矩阵行 73 补记启动兜底已删；§6 home visual / appearance select / 「仍未 complete」三处 fallback 未删表述改为已退役并指回 E1-E6 证据。仅改状态列与历史注记，不动冻结清单与施工条件定义。代码面自 a5f1b0aa 起零变更。
R2-02E Forum 凭据接缝审计与 reload-restore 补证（2026-08-27，批次 23）：unlock 复查仍未满足（DirectoryBrowser 保持 `foundation-electron-active`）。按归属台账 §5 评估 Forum `adminUsername`/`adminPassword` 候选：审计结论为其施工清单大半已闭合（typed forum field owner 防抖保存、clean 投影、真 mountInput、关闭 flush/失败重试/teardown 证据齐备），残余缺口仅 journey section 7 reload-restore 断言（本批闭合：6b flush 唯一值经 reload 由 typed forum consumer 重投影）与 Harness Input reference gate 覆盖（线程 A fixture 域，evidence-pending 上报）。source-equivalence 新增论坛凭据 seam 负向守护：两 id 的 `getElementById` 写入方集合必须等于 manager 单元素（Classic 兜底）、bridge 必须经 querySelector 持有投影、save 必须经 forum config service、manager 门禁变量必须存在。生产代码零变更；四快门禁 + Electron journey 全轮 PASS。
R2-02E 兜底退役后 Settings-only stress 复跑（2026-08-27，批次 24，docs-only）：unlock 复查仍未满足。批次 20 属 mount/打开路径变更，按协议复跑 settings stage 生命周期压测（3 warmup + 20 cycles）通过：五 checkpoint listeners=831 / lifecycleResources=720 / enhancedSettingsControls=40 全量恒定、detached 三类均为 0、listenerTrace/staleWeakNodes 空——兜底退役未引入重复挂载或监听累积，typed 投影 + retired owner 拆分在重复打开/teardown 压力下资源面稳定。
R2-02E 兜底退役后 §6 门禁全集补跑（2026-08-27，批次 25，docs-only）：unlock 复查仍未满足。批次 20-24 期间仅复跑四快 + journey + settings stress，本批补齐 §6 其余四项并全绿——check:uiux（tsc）、test:uiux 50/50、artifact consistency 78 files + artifact smoke、guard:classic-retirement 边界门禁：兜底退役未越出 classic 退休边界，generated artifact 与 adapter 契约面无回归。
R2-02E legacy 整表 save 链存活状态审计（2026-08-27，批次 26，docs-only）：unlock 复查仍未满足（线程 A 交错推进 agent-settings 按钮）。审计确认 legacy 整表 save 链（mountSettingsAutosave 400ms 防抖 → requestSubmit → handleSaveGlobalSettings 整表 collect → IPC）在 next 模式存活且为设计内职责——它是 §3 冻结 38 键的唯一保存路由，typed/forum 字段经 suppress 标记与 save-result 归属过滤排除在外，与矩阵行 66 表述吻合；冻结解除前置为线程 A capability adapter 交付，非 B 侧可独立施工。B 线程当前无剩余可独立施工项，转入 unlock 复查轮转。

R2-02E Agent stream choice slice（2026-08-27）：主界面 Agent 设置的 `#agentStreamOutputTrue/#agentStreamOutputFalse` 接入生成的 Light-DOM Choice presentation primitive。原生 radio 仍是唯一业务节点，`settingsManager` 的布尔解析、持久化与 `chatManager` 的流式参数消费不变；状态为 `production-consumer-active / visual-equivalence-pending`。仍需 Harness 同语义 DOM/geometry/interaction/screenshot、Electron reload/teardown 与字段级 legacy presentation 删除证据；不得把该设置误认为聊天流式内核改造，也不得触碰 `StreamCoordinator`/`StreamProjection`。
R2-02E Agent Settings lifecycle evidence（2026-08-27）：Agent-only Electron lifecycle stress（3 warmup + 5 measured cycles）通过。每个 checkpoint 均保持 `listeners=370`、`lifecycleActiveResources=170`、`detachedRoots=0`、`detachedOptions=0`、`enhancedSettingsControls=43`；常用 Agent Input/Toggle/Choice 的 production consumer 没有重复挂载或遗留节点。该证据只闭合 owner/lifecycle 稳定性，不提升 Harness DOM/geometry/pixel 等价等级。
R2-02E Agent Settings production capture（2026-08-27）：在真实 Electron Agent Settings Surface、`800×600@1x` 下生成 `reports/vcp-agent-settings-production.json` 与 `reports/vcp-agent-settings-production.png`。报告包含完整 `agentSettingsForm` Light DOM、Input/Choice/Toggle/原生 radio 的 rect 与 computed-style；截图用于后续与 Harness 同语义 fixture 做 DOM/geometry/pixel 对照。目前该报告只是 VCP production baseline，不能单独证明 Harness 等价。
R2-02E Agent Input geometry correction（2026-08-27）：production capture 发现旧 Agent CSS 以高 specificity 强制 native input `34px/16px`，与 Harness Input contract 冲突。typed Input primitive 现在在自身 owner 生命周期内以可恢复 inline priority 固定内部 control 为 `22px` line box、`padding 0 10px`、`border-radius: 0`，由外层 `32px/8px` wrapper 持有边界；dispose 精确恢复原 inline style。新的 evidence gate 已将该 geometry 作为硬契约，仍不代表完整 pixel equivalence。
R2-02E Agent TTS speed Range slice（2026-08-27）：`#agentTtsSpeed` 与既有 `#ttsSpeedValue` 输出节点接入生成的 Light-DOM Range primitive；原生 range、数值范围、持久化和 TTS 业务消费不变，legacy Range enhancer 在该节点上跳过以避免双重 owner。Agent-only Electron stress 通过，状态为 `production-consumer-active / visual-equivalence-pending`；尚需同语义 Harness Range 几何/交互/截图证据。
R2-02E Agent TTS voice Select slice（2026-08-27）：`#agentTtsVoicePrimary` / `#agentTtsVoiceSecondary` 复用现有 `mountHarnessSelects()` 的 typed Select、portal 和动态 options observer；native select 仍是唯一业务源，旧 Select enhancer 对已挂载节点跳过。Agent-only Electron stress 现在明确断言 2 个 voice select 均为 typed projection，状态为 `production-consumer-active / visual-equivalence-pending`；仍需 Harness 同语义 Select DOM/geometry/pixel 证据。
R2-02E Agent Settings production evidence refresh（2026-08-27）：真实 Electron capture 现在同时登记 `7` 个 typed Input、`2` 个 typed Toggle、`1` 个 typed Choice（`2` 个 native radio option）、`1` 个 typed Range（保留 native range source）和 `2` 个 typed Select（保留 native select source），固定 viewport 为 `800×600@1x`。`check:agent-settings-production-evidence` 已将这些数量与业务节点存在性设为硬门禁；Agent-only lifecycle stress（3 warmup + 1 measured cycle）保持 `listeners=456`、`lifecycleActiveResources=266`、`detachedRoots=0`、`detachedOptions=0`。这只证明真实 consumer 与 owner/lifecycle 稳定，不提升 Harness DOM/geometry/pixel 等价等级；各控件仍为 `production-consumer-active / visual-equivalence-pending`，模型 picker 仍保持 legacy production modal + Candidate-only contract。
R2-02E Agent Input all-node contract audit（2026-08-27）：`diff:harness-vcp-agent-input` 不再只检查首个字段，而是逐一核对 `agentNameInput`、`agentModel`、`agentTemperature`、`agentContextTokenLimit`、`agentMaxOutputTokens`、`agentTopP`、`agentTopK` 七个 Input 的 Harness wrapper/native geometry。当前 `capturedInputCount=7`、`allNodesContractPass=true`；这仍只是共享 Input contract 通过，不代表完整 Agent Settings Surface 的 DOM/CSS/pixel 等价。
R2-02E Agent voice Select interaction evidence（2026-08-27）：新增显式命令 `npm run test:electron-agent-settings-interaction`，在跳过破坏性预检的稳定 scaffold 中对真实 `#agentTtsVoicePrimary` 执行 portal open → `body` owner / `role=menu` → Escape close → trigger focus restore。该路径通过 3 warmup + 1 measured cycle，且 lifecycle 指标保持 `listeners=456`、`lifecycleActiveResources=266`、`detachedRoots=0`、`detachedOptions=0`。这是 Select 的 production interaction evidence，不代表 Harness 同语义 DOM/CSS/pixel 等价，也不改变默认 stress 的预检流程。
R2-02E Agent voice Select evidence persistence（2026-08-27）：交互命令启用 production capture 后，`reports/vcp-agent-settings-production.json` 会持久化 `agentSelectInteraction={opened:true,menuOwner:true,role:"menu",closed:true,focusRestored:true}`；`check:agent-settings-production-evidence` 在报告存在该字段时执行完整结构断言，避免仅依赖终端日志。
R2-02E Agent control identity gate（2026-08-27）：production evidence gate 现在按 canonical id 集合校验 Agent Settings 高频控件，而不只检查数量：7 个 Input、2 个 Toggle、2 个 stream Choice radio、1 个 TTS Range、2 个 TTS voice Select。这样可阻止错误控件复用或重复挂载被误记为生产接入；通过结果仍不提升整体 Harness pixel-equivalence 等级。
R2-02E Agent action Button completion（2026-08-27）：`#resetAvatarColorsBtn` 纳入生成 Light-DOM Harness Button presentation，保留原生 `type="button"`、既有颜色回退逻辑和 canonical DOM 节点；production evidence gate 同时校验该按钮的 canonical id、Harness class 与非提交语义。它与其他 Agent action controls 一样仍处于 `production-consumer-active / visual-equivalence-pending`，尚未证明 Harness DOM/geometry/pixel 等价。
R2-02E Agent prompt mode Button slice（2026-08-27）：`#systemPromptContainer .prompt-mode-button` 的文本/模块/预制三枚模式按钮纳入生成 Light-DOM Harness Button presentation；模式切换、prompt editor 内容替换和持久化语义仍由 `settingsManager` 保持。Electron production capture 将其作为独立 `promptButtons` 集合校验三枚 no-id native buttons；状态为 `production-consumer-active / visual-equivalence-pending`，尚需 active/keyboard/disabled 与 Harness 同语义 DOM/geometry/pixel 证据。
R2-02E Agent prompt mode interaction evidence（2026-08-27）：在 Agent-only Electron journey 中对 prompt mode Button 执行异步 `original → modular → original` 序列，确认 active class 与 `promptContentContainer` 模式同步，结果记录为 `agentPromptInteraction={available:true,switched:true,restored:true}`；该证据只闭合 production interaction/lifecycle，不提升 Harness DOM/geometry/pixel 等价等级。
R2-02E Agent prompt mode ARIA state（2026-08-27）：PromptManager 为三枚 prompt mode native Button 补齐 `type="button"` 与 `aria-pressed` 状态，并在 active mode 更新时同步切换；production capture/gate 现在固定初始状态为 `true,false,false`，异步切换 journey 继续验证 active 与内容模式恢复。业务 prompt mode/persistence 语义未改变。
R2-02E Agent section disclosure audit（2026-08-27）：Agent 设置六个 section 的 header/toggle 仍由 `settingsManager.createSectionController()` 单一 legacy owner 管理，包括折叠状态持久化、摘要异步重建和 MutationObserver。当前不接入 typed DisclosureRow，避免复制 `uiCollapseStates` 或形成第二个 owner；状态记录为 `legacy-owned / migration-not-started`，待建立可取消的 typed section owner 后再迁移。
R2-02E Agent TTS regex Input slice（2026-08-27）：`#agentTtsRegexPrimary` 与 `#agentTtsRegexSecondary` 纳入生成 Light-DOM Harness Input presentation，generic legacy Input enhancer 对两 id 跳过；regex 值、持久化和 TTS 匹配语义保持 `settingsManager` canonical。production capture/gate 现将九个 Agent typed Input 按 canonical id 集合校验，状态为 `production-consumer-active / visual-equivalence-pending`。
R2-02E Agent Button geometry diff（2026-08-27）：新增 `npm run diff:harness-vcp-agent-buttons`，对真实 Electron capture 中的 action bar 与 prompt mode Buttons 逐属性比对 Harness `button.geometry.json`。当前 authored display rule、gap、radius、font、line-height、sm/md height 均有证据；computed diff 仍记录 `computed-style-mismatch`（部分 `display:flex` 表现及 legacy action bar `37px` 高度），因此 Button 组保持 `production-consumer-active / visual-equivalence-pending`。
R2-02E Agent Button geometry enforcement（2026-08-27）：Button primitive 现在由 owner-bound inline priority 统一固定 Harness 核心几何（display、gap、sm/md height、padding、radius、font-size、line-height），dispose 精确恢复原始 inline declarations。新的 Electron capture 已确认 legacy action bar 的 md 高度由 `37px` 收敛至 `36px`，sm 高度保持 `28px`；computed `display` 仍在报告中显示为 `flex`，但 authored rule 与 inline declaration 均正确，需后续独立定位运行时计算差异。
R2-02E Agent Model Picker Candidate Electron capture（2026-08-27）：新增 `capture:vcp-agent-model-picker-candidate` 与 `check:vcp-agent-model-picker-candidate-evidence`。在真实 Electron generated artifact 页面固定 `800×600@1x` 采集 root/model/effort 三层 pane、28px trigger、搜索、3 个模型选项、2 个 effort 选项与 dispose 后撤回；报告明确 `productionConsumer=false`、`status=candidate-interaction-active`。该 Candidate 仍不接入 `agentSettingsForm`、`modelSelectModal`、IPC 或持久化，不得晋级 Stable。
R2-02E Model Picker 双轨证据校准（2026-08-28）：VCP Candidate capture 现同时保存整页上下文与打开状态下的 menu ROI，避免将跨页面截图误用于像素比较；ROI 尺寸已可读取，但 pixel gate 仍诚实保持 `pending-semantic-fixture-alignment`。Harness production capture 补齐真实 effort pane、Escape/关闭/焦点状态，并明确 `searchVisible=false`。审计确认 Harness 是 2 个 provider-grouped `button[role=menuitemradio]` 且无 search，VCP Candidate 是 3 个扩展选项、search 与 generic `role=option`；因此后续必须分离 `harness-equivalent` 与 `vcp-enhanced` 两条 fixture，先对齐 primitive DOM/group contract，再进入 pixel equivalence，不得用 fixture 数量或阈值掩盖结构差异。
R2-02E Model Picker geometry closure（2026-08-28）：`harness-equivalent` fixture 已通过 `overflow-x:hidden` 与 Harness authored group gap 修正横向滚动条高度偏差，VCP menu ROI 从 `250×146` 收敛至 `250×142`，与 Harness 尺寸一致。DOM/语义对照通过；pixel gate 仍为 `compared/pass=false`，因为文字与 material token 的像素渲染尚未等价，不能晋级 Stable。
R2-02E Model Picker stacking diagnosis（2026-08-28）：真实 Electron capture 在菜单中心点执行 `elementFromPoint`，topmost 命中底层 `.chat-messages-container` 而非菜单；菜单自身 `z-index:100`，但候选 host 的祖先 stacking context 为 `z-index:auto`。该结果将像素污染归因到可复现的 stacking/cascade 缺陷，待 Visual Forensics 线程在不改变聊天冻结边界的前提下提供最小层级修复与回归截图。
R2-02E Visual Forensics baseline refresh（2026-08-28）：`npm run check:visual-forensics` 在 light/dark 两主题及 `800×600`、`1280×800`、`1680×1000` 六组固定 viewport 全部通过；该门禁只证明 VCP 页面自身基线稳定，不覆盖 Harness 跨页面像素等价，也不抵消 Model Picker topmost stacking 缺陷。
R2-02E Model Picker typography convergence（2026-08-28）：按 Harness `ModelSelect.module.css` 将 parity option label 收敛为 `14px/20px/500`；同尺寸 pixel diff 的 differing ratio 从 `9.75%` 降至 `8.09%`、mean channel delta 从 `9.07` 降至 `3.85`。仍未达到 pixel policy，后续继续处理 material/token 与剩余 cascade 差异。
R2-02E Model Picker isolated ROI recheck（2026-08-28）：候选 capture host 现在显式使用独立高层级 stacking context，避免真实聊天页面覆盖 synthetic ROI；`elementFromPoint` 已命中 `.vcp-harness-popup-select-viewport`，同尺寸截图保持 `250×142`。该隔离只修正实验室测量条件，不改变生产层级；生产 stacking 缺陷仍需单独 QA 修复。
R2-02E Model Picker raster alignment probe（2026-08-28）：capture 线程将 synthetic host 的水平 origin 调整到与 Harness clip 对齐的 device-pixel 边界后，same-size pixel diff 从 `8.09% / 3.85` 收敛至 `6.32% / 1.80`（differing ratio / mean channel delta）。平均色差已接近 policy，但 differing ratio 仍超过 `1%`，因此继续保持 pixel-equivalence-pending。
R2-02E Model Picker group-title token correction（2026-08-28）：Harness group title 的实测主色为 `rgb(129,133,140)`，而 VCP inherited tertiary token 为 `rgb(115,119,128)`；parity-only selector 已锁定 Harness authored 值，same-size pixel diff 收敛至 `6.18% / 1.70`。该修正只作用于 Harness-equivalent fixture，enhanced 模式与生产业务 token 不变，pixel policy 仍未通过。
R2-02E Agent Model Picker lifecycle recheck（2026-08-28）：重新运行 `npm run test:electron-agent-model-picker`，open/pane/Escape/focus/dispose journey 通过；baseline 与 measured checkpoint 均保持 `listeners=472`、`lifecycleResources=325`、`detachedRoots=0`、`detachedOptions=0`。该证据确认近期 geometry/typography 修正没有引入生命周期泄漏，但不提升 pixel-equivalence 或 legacy retirement 状态。
R2-02E Agent Settings production interaction recheck（2026-08-28）：`check:agent-settings-production-evidence` 与 `test:electron-agent-settings-interaction` 均通过；真实 Agent Settings 报告记录 9 Inputs、2 Toggles、1 Choice group、1 Range、2 Selects，以及 Model Picker 的 open/filter/select/reopen/Escape/focus/dispose 状态。stress checkpoint 保持 `listeners=472`、`lifecycleResources=325`、无 detached roots/options；该证据闭合 production interaction/lifecycle，不改变 Harness pixel-equivalence 或 legacy modal retirement 结论。
R2-02E Harness evidence gate recheck（2026-08-28）：`build:uiux`、`check:uiux:artifacts`、`check:harness-contracts`、`check:harness-fixture-matrix` 与 `tests/harness-parity-evidence.test.mjs` 全部通过（fixture matrix：135 visual cases、29 interaction cases、5 output layers；parity audit 13/13）。该批次确认 reference/fixture/provenance 门禁无回归，仍保留 Model Picker pixel mismatch、source-only controls 与 frozen-domain boundaries。
R2-02E UIUX full test recheck（2026-08-28）：主线 `npm run test:uiux` 通过 60/60，覆盖 DomRenderer、B1/B2 primitives、service registry、Settings adapters、ThemeTokenOwner 与 CSS module boundary；使用 paired Harness-equivalent reports 的 Model Picker pixel runner 仍为 `semanticEquivalent=true`、`compared/pass=false`（约 `6.19%` differing ratio、`1.77` mean delta），默认 enhanced fixture 则按策略保持 pending。
R2-02E generated artifact sync checkpoint（2026-08-28）：随后一次 `check:harness-fixture-matrix` 与 `check:harness-reference` 通过（140 visual cases、30 interaction cases、89 reference files、43 primitive contracts），但 `check:uiux:artifacts` 捕获到并行修改后的 `agent-model-picker.ts` 与 generated `agent-model-picker.js` 不一致。该门禁按设计阻断后续 artifact smoke；源码归属仍在 Model Picker 并行线程，待其重新生成并提交产物后复跑。
R2-02E Model Picker check-marker contract recheck（2026-08-28）：parity check marker 已按 Harness `.check` 收敛为 `display:grid; place-items:center; flex:0 0 18px`，source/generated artifact 与 focused primitive tests 通过；pixel 指标保持约 `6.19% / 1.77`，说明剩余差异不在 check 容器布局，继续保持 visual-equivalence-pending。
R2-02E Model Picker cascade debt cleanup（2026-08-28）：提交 `8ba13666` 将已由 `AgentModelPicker` 接管的 trigger 从旧 `.model-input-container` 与 legacy SVG selector 排除，消除生产页面对 trigger 的旧 authored `display:flex`/尺寸覆盖；Visual Forensics 的 VF-001 cascade 与 VF-002 reopen/Escape 缺陷已标记 resolved。`npm run check:uiux`、`npm run check:agent-settings-production-evidence`、`npm run test:electron-agent-model-picker` 与 chat-kernel guard 通过；严格同引擎 pixel diff 仍约 `6.19%`，不得晋级 Stable。
R2-02E PermissionRow reference audit（2026-08-28）：新增 `check:harness-permission-row-reference`，对 Harness source/CSS provenance 执行 11 项 geometry/token 检查并通过。PermissionRow 当前仍是 `candidate-source-only`：没有 VCP production consumer、paired browser capture、computed-style diff 或 pixel diff，不得将 reference audit 误写为生产等价。
R2-02E lifecycle cleanup gate correction（2026-08-28）：修复 Agent Settings Electron stress 的 optional `modelServer` 关闭 Promise；server 为空时不再创建永不 resolve 的 Promise，避免 Node unsettled top-level await 导致命令打印 passed 但以 exit 13 结束。修复后 Agent Settings interaction 命令真实 exit 0，listeners/resources 与 detached-node 指标稳定。
R2-02E JobListAction reference audit（2026-08-28）：新增 `check:harness-job-list-action-reference`，对 Harness live-first/newest-settled ordering、open-only ticking、outside-dismiss、Escape focus restore 与 disposal 相关 CSS 几何执行 21 项来源审计并通过；JobListAction 仍为 `candidate-source-only`，暂无 VCP jobs production consumer、browser capture 或 pixel equivalence，不能晋级 Stable。
R2-02E ProducedFiles frozen-domain reference audit（2026-08-28）：新增 `check:harness-produced-files-reference`，对 Harness ProducedFiles 的 basename/title、prefix fitting、overflow count、ResizeObserver 与 open-folder capability 相关 CSS 执行 23 项来源审计并通过；该控件继续标记为 `frozen-domain-reference`，不得接入 VCP 聊天 turn-tail、工具结果或附件渲染，也没有 VCP production consumer 或 pixel equivalence 证据。
R2-02E reference-pack checkpoint（2026-08-28）：`npm run check:harness-reference` 通过（93 个 reference files、45 个 primitive contracts），`check:uiux:artifacts` 通过（78 个 generated files），fixture matrix 保持 150 visual / 32 interaction。该 checkpoint 只证明 provenance/contract/artifact 平面完整，不提升任何 source-only candidate 为 production-equivalent 或 Stable。
R2-02E MessageImage frozen-domain reference audit（2026-08-28）：新增 `check:harness-message-image-reference`，对 Harness MessageImage 的 durable URL、retry/lightbox handoff、thumbnail ARIA、single/tile fit 与 stale-resolution 生命周期相关 CSS 执行 30 项来源审计并通过；该控件继续标记为 `frozen-domain-reference`，VCP 聊天附件渲染保持冻结，没有 production consumer、paired visual capture 或 pixel equivalence 证据。
R2-02E LanguageRow WIP risk audit（2026-08-28）：LanguageRow 的 source/reference 文件仍由并行线程持有且未提交；当前 `check:uiux`、artifact、reference、fixture 与 focused primitive tests 通过，但异步 `setOptions()` 重建尚未证明 generation protection，存在旧重建结果覆盖新 options 的风险。继续保持 `candidate-source-only`，待归属线程补并发/取消测试和同语义视觉证据后再验收，不接入 Settings 或新增 locale durable state。
R2-02E WelcomeNotice Button ROI recheck（2026-08-28）：`npm run diff:harness-vcp-button-welcome` 重新执行并通过完整 DOM/geometry/computed-style 合同与严格 RGBA policy（`37/4320` differing pixels、`0.856%`、mean channel delta `0.051`，低于 `1%/2` 门限）。该结果只证明 Harness WelcomeNotice 与 VCP Candidate 的同引擎 Button ROI 视觉接近；VCP production consumer 与 legacy deletion 仍未闭合，状态保持 `candidate-button-roi-pixel-policy-pass`，不得晋级 Stable。
R2-02E ImageGallery frozen-domain reference audit（2026-08-28）：新增 `check:harness-image-gallery-reference`，对 Harness ImageGallery 的 single/tile aggregation、thumbnail alignment、overflow/fit 与 lightbox handoff 相关 CSS 执行 12 项来源审计并通过；该控件继续标记为 `frozen-domain-reference`，VCP 聊天附件/图片渲染保持冻结，没有 production consumer、paired visual capture 或 pixel equivalence 证据。
R2-02E PlanChip frozen-domain reference audit（2026-08-28）：新增 `check:harness-plan-chip-reference`，对 Harness PlanChip 的状态投影、label/icon 对齐、pending/complete/error 几何与 token 相关 CSS 执行 18 项来源审计并通过；该控件继续标记为 `frozen-domain-reference`，VCP 思维链/聊天内容渲染保持冻结，没有 production consumer、paired visual capture 或 pixel equivalence 证据。
R2-02E Agent Model Picker single-owner cleanup（2026-08-28）：`settings-bridge` 不再把 `#openModelSelectBtn` 同时注册为通用 Button 与 `AgentModelPicker` trigger；ModelPicker 现在是该节点唯一 presentation/lifecycle owner，保存、删除、TTS 刷新和颜色重置按钮仍由 Button primitive 管理。production evidence 将 trigger 单独按 picker contract 校验，避免双 owner 被误记为 Button 覆盖。`npm run check:agent-settings-production-evidence`、`npm run test:electron-agent-model-picker`、`npm run check:uiux`、`npm run check:uiux:artifacts`、UIUX focused tests（35/35）与 `guard:chat-kernel-consumers` 全部通过；stress 的 listeners 仍为 `472`，lifecycle resources 从 `327` 降至 `324`，detached roots/options 为 `0`。提交 `8fad4424`。Model Picker 同引擎 pixel diff 仍约 `6.19% / 1.77`，legacy hot/favorite/refresh modal 能力和 Stable 晋级继续 pending。
R2-02E Agent Model Picker single-owner cleanup（2026-08-28）：提交 `8fad4424` 从通用 `mountTypedAgentButtons` 移除 `#openModelSelectBtn`，使 `AgentModelPicker` 成为该 trigger 的唯一 presentation owner；production evidence checker 改为独立核验 picker trigger。`check:agent-settings-production-evidence`、`test:electron-agent-model-picker`、`check:uiux`、artifact consistency 与 chat-kernel guard 通过，listeners `472`、lifecycle resources `324` 稳定；Model Picker 严格 pixel diff 仍 pending，Settings/legacy modal 也未整体完成。
R2-02E Agent action geometry/disposer cleanup（2026-08-28）：legacy action CSS 的 `min-height:37px` 会把 typed Button 的 md 高度推成 `37px`；`settings-bridge` 现由同一 action owner 施加 `min-height:36px`（sm 为 `28px`），并在 teardown 恢复原声明。由于 `mountButton()` 已自行注册 scope disposer，bridge 同时移除重复 `scope.own(release)`，避免每个按钮重复生命周期资源。串行 Electron capture 证明 submit/delete rect 均为 `36px`，listeners `472` 恒定、lifecycle resources `319` 恒定、detached roots/options 为 `0`；`diff:harness-vcp-agent-buttons` 过滤已由 ModelPicker 独占的 trigger 后，7 个真正 Button consumer 的 authored contract 通过，computed display mismatch 仍如实保留。提交 `c52057c2`，Model Picker pixel/legacy modal retirement 继续 pending。
R2-02E LanguageRow race closure（2026-08-28）：提交 `5d05e775` 为 LanguageRow 的异步 `setOptions()` 增加串行 rebuild queue、generation stale-publication guard 与 dispose quiescence；新增 concurrent option replacement 和 dispose race 测试，并同步 generated artifact 与 reference contract。`build:uiux`、`check:uiux`、`check:uiux:artifacts`、`check:harness-reference`、`check:harness-fixture-matrix`、artifact smoke 与 `test:uiux`（63/63）通过。LanguageRow 仍为 `candidate-source-only`，没有 locale production consumer 或 pixel evidence。
R2-02E Tooltip parallel regression checkpoint（2026-08-28）：并行 Tooltip 修改尚未提交；其当前 Electron theme journey 出现 `tooltipOpen=false` 而非预期 `true`，因此不能把该改动纳入稳定证据或提交。其它 UIUX/reference 门禁通过，等待 Tooltip 线程修复并重新生成 artifact、恢复 theme journey 后再验收。
R2-02E JsonTree/ReadBlock/SearchBlock frozen-domain reference audits（2026-08-28）：新增三条 source/CSS provenance 门禁；ReadBlock 通过 `28/28` CSS checks，SearchBlock 通过 `29/29` CSS checks，JsonTree reference audit 与 parity evidence 通过，parity evidence 总计 `22/22`。三者均无 VCP production consumer、paired visual/pixel evidence，继续标记为 `frozen-domain-reference`，不得触碰聊天内容、工具结果或消息渲染冻结边界。
R2-02E TerminalBlock frozen-domain reference audit（2026-08-28）：新增 `check:harness-terminal-block-reference`，对 Harness TerminalBlock 的 command/output 分区、折叠状态、滚动与 geometry/token provenance 执行 32 项来源审计并通过；该控件继续标记为 `frozen-domain-reference`，VCP 工具结果与聊天内容渲染保持冻结，没有 production consumer 或 pixel equivalence 证据。
R2-02E Agent Button disposer cleanup（2026-08-28）：提交 `c52057c2` 清理 Agent Settings 通用 Button 的重复 disposer，保留单一 owner 的 teardown 路径；UIUX focused tests 与 Agent Settings evidence 继续通过，未改变聊天核心或持久化语义。Tooltip 并行改动仍导致 generated artifact stale，待其归属线程完成 source/artifact 同步与 Electron theme journey 回归。
R2-02E Tooltip fragment-sibling contract closure（2026-08-28）：提交 `a402f225` 恢复 Harness Tooltip 的 fragment-sibling bubble 挂载，移除错误的 `document.body` portal 语义，并新增 parent-sibling 回归断言；`test:uiux`（63/63）、build、artifact consistency 与 Tooltip focused tests 通过。完整 Electron theme journey 已越过 Tooltip，随后仍被并行 AgentPresetRow 的 label 断言阻断（实际 `BalancedGeneral purpose model` 与旧预期 `Balanced` 不一致），该失败不归因于 Tooltip。
R2-02E Tooltip scroll-tracking regression checkpoint（2026-08-28）：后续并行 Tooltip 变更新增 capture-phase document scroll listener；当前未提交 source/generated 变更重跑 `test:electron-uiux-theme` 时，candidateLab 仍报告 `tooltipOpen=false`、`tooltipSide=""`，说明新变更尚未满足现有真实 journey 合同。fragment-sibling 修复保持正确，待 Tooltip 线程完成 source/artifact 同步并补齐真实 open/focus/scroll 回归后再验收。
R2-02E Tooltip sibling + scroll closure（2026-08-28）：提交 `470bec60` 将 Tooltip bubble 固定在 anchor 的 fragment sibling，同时保留 owner-bound scroll/resize reposition 与释放逻辑；`test:uiux`（63/63）和 `check:uiux:artifacts`（78 files）通过。完整 Electron theme journey 已通过 Tooltip candidate 段，当前剩余失败是并行 AgentPresetRow label contract（实际 `BalancedGeneral purpose model`，旧断言为 `Balanced`），不归因于 Tooltip。
