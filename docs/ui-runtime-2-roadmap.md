...
2026-08-26 fixture matrix scaffold：新增 `fixture-matrix.json` 与 `npm run check:harness-fixture-matrix`，锁定 800×600、DPR 1、system-ui、Input/Field/Select 九个状态及 DOM/geometry/computed-style/screenshot/pixel-diff 五层输出；当前状态为 `harness-dom-captured-vcp-capture-pending`，未把矩阵定义误报为 Harness 参考图或 pixel diff 完成。
